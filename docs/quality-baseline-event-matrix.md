...
occurrence fingerprints, with Python-version-independent AST scope evidence, stable adjacent local
anchors, and line-shift-safe occurrence ordering. The scope digest identifies enclosing
class/function names without hashing mutable scope bodies; local anchors distinguish same-scope
replacements. Fragment normalization preserves quoted literal contents while collapsing layout
whitespace.
Detector
