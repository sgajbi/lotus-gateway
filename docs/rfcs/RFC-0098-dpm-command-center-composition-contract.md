...
| RFC-0042 manage authority preservation | Implemented across all DPM mutation route families; Gateway preserves caller audit identity and derives only its own bounded Manage workload authority | Request-scoped policy covers every DPM `POST`, `PUT`, `PATCH`, and `DELETE`; client/router/OpenAPI tests prove exact `lotus-gateway` / `manage.write` authority, hostile-header replacement, missing/invalid caller failure, least-privilege reads, payload preservation, and upstream error forwarding |
