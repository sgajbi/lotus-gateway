...
| RFC41-WTBD-003 Gateway campaign-definition discovery/upsert/launch | Implemented in current Gateway branch; pending PR/merge/CI/wiki closure | Gateway forwards manage-owned campaign definition list, get, lifecycle-events, launch-history, launch-package, launch, upsert, and bounded campaign-discovery APIs through `/api/v1/dpm/command-center/waves/campaign-definitions*` and `/api/v1/dpm/command-center/waves/campaign-discovery`, preserves campaign payloads, launched lifecycle events, append-only launch history, launch readiness, durable wave response, reason codes, idempotency evidence, and replay posture unchanged, and does not discover cohorts beyond the Manage-owned persisted campaign read model, recompute campaign membership, launch readiness, or idempotency, own maker-checker posture, approve trades, stage orders, route orders, or claim OMS execution |
