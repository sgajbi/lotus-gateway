...
| RFC41-WTBD-003 Gateway campaign-definition discovery/upsert/launch/lifecycle commands | Implemented and merged on Gateway `main` for the bounded BFF surfaces; broader RFC41 campaign workflow/global campaign depth remains out of scope unless separately approved. | Gateway forwards manage-owned campaign definition list, get, lifecycle-events, preview-readiness, paged launch-history, launch-package, launch, retire, supersede, upsert, and bounded campaign-discovery APIs through `/api/v1/dpm/command-center/waves/campaign-definitions*` and `/api/v1/dpm/command-center/waves/campaign-discovery`, preserves campaign payloads, launched lifecycle events, retire/supersede lifecycle lineage, replacement version/hash evidence, `BulkReviewCampaignDefinitionPreviewReadiness:v1` supportability state/reason codes/blocked actions/source refs/operating boundaries, `BulkReviewCampaignDefinitionLaunchHistory:v1` items/counts/pagination/operating boundaries, launch readiness, durable wave response, reason codes, idempotency evidence, and replay posture unchanged, and does not discover cohorts beyond the Manage-owned persisted campaign read model, recompute campaign membership, preview readiness, launch readiness, lifecycle state, or idempotency, infer actor entitlement, own maker-checker posture, approve trades, stage orders, generate orders, route orders, claim fills/settlement, or claim OMS execution |
| RFC41-WTBD-006 Gateway campaign workflow/audit composition | Implemented for bounded BFF pass-through; typed write-boundary hardening is issue-backed by Gateway #567 and paired Workbench realization by Workbench #772. | Gateway forwards manage-owned campaign operating queue, approval inbox, workflow board, assignment plan, workflow automation, approval-decision list/create, assignment-action list/create, assignment-task list/create, task-transition create, and maker-checker-control list/create APIs through `/api/v1/dpm/command-center/waves/*`; validates each write family with a distinct closed request schema and current bounded vocabulary; preserves payloads, count/page metadata, supportability, source refs, reason codes, operating boundaries, content hashes, no-order/no-OMS/no-client-contact/no-external-workflow posture; and does not calculate command eligibility, cohort membership, readiness, idempotency, task state, maker-checker state, approval state, order/execution state, SLA posture, escalation, or workflow orchestration locally |
...
| RFC42-WTBD-008 Gateway PM operating quality composition | Implemented and merged on Gateway `main`; current repo/wiki truth reflects policy, score-run, fairness-analysis, review-action, and support-only AI summary surfaces. | Gateway forwards manage-owned PM operating quality policy list/get/upsert, score-run preview/create/list/get, fairness-analysis preview/create/list/get, and review-action preview/create/list/get APIs through `/api/v1/dpm/command-center/pm-operating-quality/*`; reads Manage-owned score-run evidence before invoking `lotus-ai` `pm_quality_summary.pack@v1`; preserves policy configuration, score-run state, fairness-analysis state, review-action state, bounded rationale, target content hashes, segment posture, governance evidence, source refs, reason codes, content hashes, supportability, and forbidden-use posture; and does not calculate scores, discover segments, calculate segment averages or fairness spread, infer protected classes, rank PMs, administer policy locally, reinterpret review rationale, or create HR, compensation, conduct-enforcement, approval, client-contact, OMS, or execution decisions |
...
| RFC-0042 manage authority preservation | Implemented across all registered DPM route families; Gateway preserves caller audit identity on reads and derives only its own bounded Manage workload authority on mutations | Request-scoped policy covers every DPM read and mutation; client/router/OpenAPI tests prove caller-context propagation without read capabilities, exact mutation-only `lotus-gateway` / `manage.write` authority, hostile-header replacement, missing/invalid caller failure, payload preservation, and upstream error forwarding |
...
14. Wave routes preserve manage-owned `wave_id`, lifecycle state, item states, reason codes,
   ...
   definition payloads, launched lifecycle events, retire/supersede lifecycle command evidence,
   preview-readiness supportability, append-only launch history, launch-package readiness, launch wave responses, campaign workflow/audit
   payloads, count/page metadata, source refs, content hashes, supportability issues,
   source-owner remediation, replay posture, and no-order/no-OMS/no-external-workflow boundaries.
15. Gateway does not calculate affected portfolios, classify source readiness, generate
   alternatives, discover cohorts, recompute campaign membership, recompute preview readiness,
   select alternatives, approve items, stage items, create handoff evidence, rebuild proof packs,
   calculate lifecycle state, task state, approval state, maker-checker state, SLA posture, escalation, or workflow
   orchestration, or claim external execution locally.
