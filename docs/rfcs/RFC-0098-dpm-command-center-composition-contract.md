...
| RFC41-WTBD-003 Gateway campaign-definition discovery/upsert/launch | Implemented in current Gateway branch; pending PR/merge/CI/wiki closure | Gateway forwards manage-owned campaign definition list, get, lifecycle-events, launch-package, launch, upsert, and bounded campaign-discovery APIs through `/api/v1/dpm/command-center/waves/campaign-definitions*` and `/api/v1/dpm/command-center/waves/campaign-discovery`, preserves campaign payloads, launch readiness, durable wave response, reason codes, and replay posture unchanged, and does not discover cohorts beyond the Manage-owned persisted campaign read model, recompute campaign membership or launch readiness, own maker-checker posture, approve trades, stage orders, or claim OMS execution |
...
14. Wave routes preserve manage-owned `wave_id`, lifecycle state, item states, reason codes,
   ...
   definition payloads, launch-package readiness, launch wave responses, supportability issues,
