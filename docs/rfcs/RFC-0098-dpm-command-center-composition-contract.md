...
| RFC41-WTBD-006 Gateway campaign workflow/audit composition | Implemented on this branch for bounded BFF pass-through; Workbench realization follows after Gateway merge. | Gateway forwards manage-owned campaign operating queue, approval inbox, workflow board, assignment plan, workflow automation, approval-decision list/create, assignment-action list/create, assignment-task list/create, task-transition create, and maker-checker-control list/create APIs through `/api/v1/dpm/command-center/waves/*`; preserves payloads, count/page metadata, supportability, source refs, reason codes, operating boundaries, content hashes, no-order/no-OMS/no-client-contact/no-external-workflow posture, and does not calculate cohort membership, readiness, idempotency, task state, maker-checker state, approval state, order/execution state, SLA posture, escalation, or workflow orchestration locally |
