...
| RFC41-WTBD-003 Gateway campaign-definition discovery/upsert/launch | Implemented and merged on Gateway `main` for the bounded BFF surfaces; broader RFC41 campaign workflow/global campaign depth remains out of scope unless separately approved. | Gateway forwards manage-owned campaign definition list, get, lifecycle-events, preview-readiness, paged launch-history, launch-package, launch, upsert, and bounded campaign-discovery APIs through `/api/v1/dpm/command-center/waves/campaign-definitions*` and `/api/v1/dpm/command-center/waves/campaign-discovery`, preserves campaign payloads, launched lifecycle events, `BulkReviewCampaignDefinitionPreviewReadiness:v1` supportability state/reason codes/blocked actions/source refs/operating boundaries, `BulkReviewCampaignDefinitionLaunchHistory:v1` items/counts/pagination/operating boundaries, launch readiness, durable wave response, reason codes, idempotency evidence, and replay posture unchanged, and does not discover cohorts beyond the Manage-owned persisted campaign read model, recompute campaign membership, preview readiness, launch readiness, or idempotency, infer actor entitlement, own maker-checker posture, approve trades, stage orders, generate orders, route orders, claim fills/settlement, or claim OMS execution |
...
14. Wave routes preserve manage-owned `wave_id`, lifecycle state, item states, reason codes,
   ...
   definition payloads, launched lifecycle events, preview-readiness supportability, append-only
   launch history, launch-package readiness, launch wave responses, campaign workflow/audit
...
15. Gateway does not calculate affected portfolios, classify source readiness, generate
   ...
   calculate task state, approval state, maker-checker state, SLA posture, escalation, or workflow
