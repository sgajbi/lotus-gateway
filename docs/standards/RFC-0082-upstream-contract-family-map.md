...
| `lotus-manage` | versioned `/api/v1` rebalance run lookup, supportability summary, capability posture, construction alternatives, proof packs, portfolio memory, outcome reviews, report input, and AI evidence input | management workflow composition | discretionary management operations, proof-pack evidence, portfolio-memory timeline lineage, and outcome-review truth remain in `lotus-manage`; gateway must not reintroduce retired unversioned aliases, monolithic `dpm-execution-context` assumptions, or local timeline reconstruction |
