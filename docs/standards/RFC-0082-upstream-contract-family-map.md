...
| `lotus-manage` | versioned `/api/v1` rebalance run lookup, supportability summary, capability posture, construction alternatives, proof packs, portfolio memory, outcome reviews, report input, AI evidence input, and PM operating quality policy/score-run lifecycle APIs | management workflow composition | discretionary management operations, proof-pack evidence, portfolio-memory timeline lineage, outcome-review truth, and PM operating quality policy/score-run truth remain in `lotus-manage`; gateway must not reintroduce retired unversioned aliases, monolithic `dpm-execution-context` assumptions, local timeline reconstruction, PM scoring logic, PM ranking, or HR/compensation/conduct decision semantics |
