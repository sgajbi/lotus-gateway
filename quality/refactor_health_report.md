...
| Live canonical runtime | Healthy | Rebuilt `lotus-gateway` from this branch, validated `PB_SG_GLOBAL_BAL_001` through canonical Workbench/Gateway/downstream paths, and captured machine-readable evidence at `lotus-workbench/output/playwright/live-canonical-gateway-portfolio-service-boundary/live-validation-summary.json` with 29 screenshots, 25 ready panel classifications, 2 calculation checks, 28 supportability checks, 10 workflow-pack checks, and 12 advisory journey checks |
