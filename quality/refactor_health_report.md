...
| Unit/contract coverage | Healthy | Current foundation core snapshot section extraction focused validation includes Foundation snapshot mapper tests and service-layer boundary tests plus a 331-line threshold pass proving the ceiling now lands on evidence-selected `src/app/contracts/domain_products.py`; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 682 source files, OpenAPI smoke, and 1,392 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves foundation core snapshot payload validation and section extraction into `src/app/services/foundation_core_snapshot_sections.py` while preserving the public Foundation mapper behavior. The evidence-selected largest current source-file hotspot is `src/app/contracts/domain_products.py` at 331 lines |
...
1. Continue splitting large contract modules, service modules, observability modules, and remaining upstream clients
   ...
   `src/app/contracts/domain_products.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 331 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 331/49 thresholds when the current largest file or
   function improves, and to keep `src/app/contracts/domain_products.py` hotspot
