...
| Unit/contract coverage | Healthy | Current proposal delivery narrative extraction focused validation includes proposal service and service-boundary tests plus a 321-line threshold pass proving the ceiling now lands on evidence-selected `src/app/services/dpm_client_protocols.py`; full local `make check` evidence remains required before commit |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves proposal narrative read/regenerate behavior into `src/app/services/proposal_delivery_service.py`, keeping narrative delivery behavior out of the main proposal facade. The evidence-selected largest current source-file hotspot is `src/app/services/dpm_client_protocols.py` at 321 lines |
...
1. Continue splitting large contract modules, service modules, observability modules, and remaining upstream clients
   ...
   `src/app/services/dpm_client_protocols.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 321 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 321/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/dpm_client_protocols.py` hotspot
