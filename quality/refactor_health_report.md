...
| Unit/contract coverage | Healthy | Focused validation passed with 233 upstream-client and portfolio service/catalog tests, mypy over the touched analytics client/payload and portfolio service/catalog modules, and refactor-threshold trials proving `max_source_file_lines=685` passes while `684` fails on `src/app/services/workbench_service.py`; full local `make check` remains pending for this branch |
| Integration coverage | Healthy | Latest merged full local `make ci` evidence passed with migration contract smoke and 209 integration tests; this branch's full `make ci` remains pending |
| Total coverage | Healthy | Latest merged full local `make ci` evidence passed with 1,422 combined coverage tests and 94.29% total coverage; this branch's full coverage gate remains pending |
| Security audit | Governed | Current analytics/catalog boundary branch introduces no dependency, authentication, caller-context, product-error-detail, monetary-float policy, or upstream error-shape changes; this branch's full `make ci` security audit remains pending |
