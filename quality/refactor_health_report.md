...
| Unit/contract coverage | Healthy | Current Foundation optional-workspace focused validation passed with 30 foundation/quality tests and refactor-threshold proof at `max_source_file_lines=589`; full local `make check` passed with workflow governance, mypy over 566 source files, OpenAPI smoke, and 1,249 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits Foundation optional-workspace parsing into a focused module while preserving the public service surface. The largest current source-file hotspot is now `src/app/services/portfolio_service.py` at 589 script-counted lines |
