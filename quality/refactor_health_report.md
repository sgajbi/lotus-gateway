...
| Unit/contract coverage | Healthy | Current performance attribution trend contract extraction focused validation includes attribution contract, contract-boundary, Workbench OpenAPI contract, monetary-float guard, refactor-threshold, quality-baseline artifact, and agent quality evidence tests; full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 651 source files, OpenAPI smoke, and 1,368 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves performance attribution trend row/response contracts into a focused contract module while preserving compatibility exports from the broader attribution module. The largest current source-file hotspot is `src/app/clients/reporting_client.py` at 360 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/clients/reporting_client.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 360 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 360/49 thresholds when the current largest file or
   function improves, and to keep `src/app/clients/reporting_client.py` hotspot
