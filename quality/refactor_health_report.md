...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits risk attribution supportability item construction into a focused service helper while preserving public attribution control and supportability behavior. The largest current source-file hotspot is `src/app/services/dpm_command_center_service.py` at 394 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 394 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 394/49 thresholds when the current largest file or
