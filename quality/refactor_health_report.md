...
| Unit/contract coverage | Healthy | Current platform capability service cleanup focused validation includes platform capability service, normalization, and service-boundary tests plus a 324-line threshold pass proving the ceiling now lands on evidence-selected `src/app/services/proposal_service.py`; full local `make check` evidence remains required before commit |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch removes legacy private pass-through helpers from `src/app/services/platform_capabilities_service.py`, leaving feature, workflow, and module-health decisions in their owning normalization modules. The evidence-selected largest current source-file hotspot is `src/app/services/proposal_service.py` at 324 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 324 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 324/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/proposal_service.py` hotspot
