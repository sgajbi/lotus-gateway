...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits DPM command-center response envelope assembly into a focused service helper while preserving route orchestration and Manage upstream error semantics. The largest current source-file hotspot is `src/app/services/platform_capabilities_workspace_descriptors.py` at 388 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 388 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 388/49 thresholds when the current largest file or
