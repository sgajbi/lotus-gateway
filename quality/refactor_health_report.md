...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits Gateway analytics metric collectors and bounded metric recording into a focused observability helper while preserving fan-out logging, audit logging, and no-sensitive-label behavior. The largest current source-file hotspot is `src/app/services/dpm_pm_operating_quality_service.py` at 383 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/contracts/advisor_brief.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 383 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 383/49 thresholds when the current largest file or
