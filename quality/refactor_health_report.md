...
| Unit/contract coverage | Healthy | Current reporting batch client extraction focused validation includes reporting client boundary, upstream reporting batch forwarding, refactor-threshold, quality-baseline artifact, mypy, and agent quality evidence tests; full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 652 source files, OpenAPI smoke, and 1,369 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves report batch and schedule methods into a focused reporting client mixin while preserving the public client surface. The largest current source-file hotspot is `src/app/services/performance_workspace_benchmarks.py` at 359 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/services/performance_workspace_benchmarks.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 359 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 359/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/performance_workspace_benchmarks.py` hotspot
