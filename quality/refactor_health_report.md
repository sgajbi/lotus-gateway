...
| Unit/contract coverage | Healthy | Current performance workspace attribution-trend service extraction focused validation includes performance workspace service, performance attribution, service-boundary, refactor-threshold, and agent quality evidence checks, with refactor-threshold proof at `max_source_file_lines=411`; full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 615 source files, OpenAPI smoke, and 1,305 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits performance workspace attribution-trend orchestration into a focused service mixin while preserving public `PerformanceWorkspaceService` behavior. The largest current source-file hotspot is now `src/app/services/dpm_wave_ai_handoff.py` at 411 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/services/dpm_wave_ai_handoff.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 411 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 411/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/dpm_wave_ai_handoff.py` hotspot
