...
| Unit/contract coverage | Healthy | Current DPM PM operating-quality protocol extraction focused validation includes DPM command-center and service-boundary tests plus a 319-line threshold pass proving the ceiling now lands on evidence-selected `src/app/contracts/portfolio_workspace.py`; full local `make check` evidence remains required before commit |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves PM operating-quality client protocol ownership into `src/app/services/dpm_pm_operating_quality_client_protocols.py`, keeping PM operating-quality protocol requirements out of the broader DPM command-center protocol. The evidence-selected largest current source-file hotspot is `src/app/contracts/portfolio_workspace.py` at 319 lines |
...
1. Continue splitting large contract modules, service modules, observability modules, and remaining upstream clients
   ...
   `src/app/contracts/portfolio_workspace.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 319 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 319/49 thresholds when the current largest file or
   function improves, and to keep `src/app/contracts/portfolio_workspace.py` hotspot
