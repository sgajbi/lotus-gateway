...
| Unit/contract coverage | Healthy | Focused proposal service-boundary validation passed with 38 proposal-service and service-boundary tests and refactor-threshold trials proving `max_source_file_lines=646` passes while `645` fails on `src/app/contracts/advisor_brief.py`; full local `make check` remains pending for this branch |
| Integration coverage | Healthy | Full local `make ci` passed with migration contract smoke and 209 integration tests |
| Total coverage | Healthy | Full local `make ci` passed with 1,434 combined coverage tests and 94.33% total coverage |
