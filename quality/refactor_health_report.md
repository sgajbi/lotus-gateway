...
| Unit/contract coverage | Healthy | Current advisor-brief source-supportability focused validation passed with 64 advisor-brief source, supportability, service, and service-boundary tests and refactor-threshold proof at `max_source_file_lines=504`; prior full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 581 source files, OpenAPI smoke, and 1,274 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits advisor-brief source supportability into the focused supportability module while preserving the existing source-context surface. The largest current source-file hotspot is now `src/app/services/portfolio_transaction_summary.py` at 504 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 504 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 504/49 thresholds when the current largest file or
