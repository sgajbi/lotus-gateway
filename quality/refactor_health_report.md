...
| Unit/contract coverage | Healthy | Current PM operating-quality summary-invocation extraction focused validation includes DPM command-center service tests, service-layer boundary tests, a 354-line threshold failure trial proving the new 355-line ceiling, and agent quality evidence checks; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 656 source files, OpenAPI smoke, and 1,371 unit/contract tests |
...
| Security audit | Governed | Current Foundation core market-value extraction introduces no dependency, authentication, caller-context, product-error-detail, upstream error-shape, data-mesh behavior, or runtime behavior changes; monetary-float governance improves to 150 findings with 152 allowlisted after moving money parsing behind a helper without adding new float-conversion debt; prior full local `make ci` passed `pip-audit` with no known vulnerabilities after the governed `PYSEC-2026-161` exception |
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves PM operating-quality summary-invocation lifecycle orchestration into a focused service mixin while preserving inherited public methods. The largest current source-file hotspot is `src/app/contracts/reporting.py` at 355 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/contracts/reporting.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 355 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 355/49 thresholds when the current largest file or
   function improves, and to keep `src/app/contracts/reporting.py` hotspot
