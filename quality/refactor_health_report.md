...
| Unit/contract coverage | Healthy | Current risk workspace response-loading extraction focused validation includes risk workspace service, service-boundary, refactor-threshold, quality-baseline artifact, and agent quality evidence tests; full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 626 source files, OpenAPI smoke, and 1,313 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves DPM command-center and mandate pass-through methods into a focused core mixin while preserving the public service surface. The largest current source-file hotspot is `src/app/contracts/performance_attribution.py` at 361 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/contracts/performance_attribution.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 361 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 361/49 thresholds when the current largest file or
   function improves, and to keep `src/app/contracts/performance_attribution.py` hotspot
