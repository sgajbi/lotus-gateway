...
| Unit/contract coverage | Healthy | Current HTTP response payload extraction focused validation includes HTTP resilience retry and payload helper tests plus a 335-line threshold pass proving the ceiling now lands on evidence-selected `src/app/contracts/risk_workspace_rolling.py`; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 678 source files, OpenAPI smoke, and 1,388 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves HTTP response and failure payload helpers into `src/app/clients/http_response_payloads.py` while preserving the public retry helper behavior. The evidence-selected largest current source-file hotspot is `src/app/contracts/risk_workspace_rolling.py` at 335 lines |
