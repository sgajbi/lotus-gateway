...
| --- | --- | --- |
| Branch hygiene | Healthy | Current advisor-brief contract-boundary branch was created from clean `main`; no open PRs and no unmerged remote branches were present at slice start; final remote/local cleanup remains a post-merge gate |
| Unit/contract coverage | Healthy | Focused advisor-brief contract-boundary validation passed with 39 contract-boundary, threshold, Advisor Brief service/supportability, and Workbench contract tests and refactor-threshold trials proving `max_source_file_lines=639` passes while `638` fails on `src/app/services/performance_workspace_service.py`; full local `make check` passed with 1,227 unit/contract tests |
| Integration coverage | Healthy | Full local `make ci` passed with migration contract smoke and 209 integration tests |
| Total coverage | Healthy | Full local `make ci` passed with 1,436 combined coverage tests and 94.33% total coverage |
| Security audit | Governed | Current advisor-brief contract-boundary branch introduces no dependency, authentication, caller-context, product-error-detail, upstream error-shape, or monetary-float conversion changes; the monetary-float guard passes with 159 allowlisted findings, and full local `make ci` security audit found no known vulnerabilities after the governed `PYSEC-2026-161` exception |
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; recent response/request-context/parser/mapper/contract slices lower `portfolio_service.py` to 589 script-counted lines, `performance_workspace_horizon.py` to 441 script-counted lines, `workbench_service.py` to 562 script-counted lines, `lotus_analytics_client.py` to 623 script-counted lines, `advisor_brief_source.py` to 508 script-counted lines, `risk_workspace_service.py` to 632 script-counted lines, `performance_workspace_evidence.py` to 461 script-counted lines, `workbench.py` to 47 script-counted lines, `performance_workspace_service.py` to 639 lines, `advisor_brief_service.py` to 435 script-counted lines, `advisory_client_protocols.py` to 630 script-counted lines, `advisor_brief_client_protocols.py` to 63 script-counted lines, `proposal_service.py` to 520 script-counted lines, `proposal_transition_service.py` to 156 script-counted lines, `dpm_command_center_service.py` to 521 lines, `dpm_command_center_exception_summary.py` to 187 lines, `dpm_wave_service.py` to 479 script-counted lines, `dpm_wave_campaign_definitions.py` to 244 script-counted lines, `advise_client.py` to 220 script-counted lines, `advise_proposal_client.py` to 536 script-counted lines, `dpm_client.py` to 452 script-counted lines, `src/app/contracts/dpm_command_center.py` to 593 script-counted lines, `src/app/contracts/proposals.py` to 431 script-counted lines, `src/app/contracts/advisor_brief.py` to 398 script-counted lines, `src/app/contracts/advisor_brief_items.py` to 127 script-counted lines, `src/app/contracts/advisor_brief_supportability.py` to 151 script-counted lines, `src/app/contracts/portfolio.py` to 281 script-counted lines, `src/app/contracts/portfolio_workspace.py` to 503 script-counted lines, `src/app/services/foundation_service.py` to 618 script-counted lines, `src/app/contracts/performance_workspace.py` to a 79-line compatibility facade, `src/app/contracts/reporting_batches.py` to a 75-line compatibility facade, `src/app/contracts/risk_workspace.py` to 312 script-counted lines, `src/app/contracts/risk_workspace_examples.py` to 379 script-counted lines, `src/app/router_registry.py` to 632 script-counted lines, and `src/app/contracts/reporting_query.py` to a 41-line compatibility facade; `src/app/services/performance_workspace_service.py` is now the largest current source-file hotspot at 639 lines |
| Live canonical runtime | Healthy for latest clean-main proof; not rerun for this contract-only branch | Latest clean-main canonical proof passed `npm run live:validate` through canonical Workbench/Gateway/downstream paths for `PB_SG_GLOBAL_BAL_001`. Machine-readable evidence at `lotus-workbench/output/playwright/live-canonical/live-validation-summary.json` was generated at `2026-06-18T16:04:19.273Z` with 94 API checks, 29 screenshots, 25 ready panel classifications, 2 calculation checks, and no missing or non-ready panels. This branch only moves Pydantic contract classes behind a compatibility facade and does not change runtime routing, service orchestration, downstream calls, or response fields |
| Observability evidence | Healthy for latest clean-main proof; not rerun for this contract-only branch | Latest clean-main companion pack `lotus-workbench/output/observability-live/20260619-000802/observability-evidence-manifest.json` captured canonical DNS, container inventory, readiness/API samples, metrics, logs, and screenshots; 13/13 API checks returned HTTP 200, 4/4 metric checks returned HTTP 200, 14 log artifacts were captured, Prometheus reported 14/14 targets up, and Gateway logs show structured correlation/request/trace evidence |
