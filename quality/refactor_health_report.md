...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits Advisor Brief response assembly into a focused service helper while preserving public advisor-brief read and review-action behavior. The largest current source-file hotspot is `src/app/services/risk_workspace_attribution_controls.py` at 397 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 397 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 397/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/risk_workspace_attribution_controls.py` hotspot
