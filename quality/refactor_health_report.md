...
| Unit/contract coverage | Healthy | Current performance detail-currency correction focused validation includes detail dependency, view, response, failure-classification, contribution, attribution, service, upstream-client, and Workbench contract tests plus a 316-line threshold pass proving the ceiling now lands on evidence-selected `src/app/contracts/performance_workspace_details_contract.py`; full local `make check` evidence remains required before commit |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch keeps detail-currency resolution and fallback policy behind a dedicated service seam while preserving separate dependency, view, response, and failure-classification ownership. The evidence-selected largest current source-file hotspot is `src/app/contracts/performance_workspace_details_contract.py` at 316 lines |
...
1. Continue splitting large contract modules, service modules, observability modules, and remaining upstream clients
   ...
   `src/app/contracts/performance_workspace_details_contract.py`.
