...
| Unit/contract coverage | Healthy | Current portfolio projected-cashflow extraction focused validation includes portfolio service/liquidity tests, service-boundary tests, a 345-line threshold failure trial proving the 346-line ceiling remains, and agent quality evidence checks; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 670 source files, OpenAPI smoke, and 1,380 unit/contract tests |
...
| Security audit | Governed | Current portfolio projected-cashflow extraction introduces no dependency, authentication, caller-context, product-error-detail, upstream error-shape, data-mesh behavior, or runtime behavior changes; monetary-float governance remains at 150 findings with 152 allowlisted; prior full local `make ci` passed `pip-audit` with no known vulnerabilities after the governed `PYSEC-2026-161` exception |
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves projected-cashflow orchestration into `src/app/services/portfolio_projected_cashflow_service.py` and shared holdings upstream access typing into `src/app/services/portfolio_holdings_upstream.py` while preserving the public `PortfolioService` surface. The largest current source-file hotspot is `src/app/services/performance_calculation_evidence.py` at 346 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/services/performance_calculation_evidence.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 346 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 346/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/performance_calculation_evidence.py` hotspot
