...
| Unit/contract coverage | Healthy | Current portfolio workspace controls contract focused validation passed with portfolio workspace contract, contract-module-boundary, and portfolio OpenAPI contract tests, mypy over touched contract modules, and refactor-threshold proof at `max_source_file_lines=499`; current full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 583 source files, OpenAPI smoke, and 1,276 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits portfolio workspace controls into a focused contract module while preserving compatibility imports through the workspace and portfolio facades. The largest current source-file hotspot is now `src/app/contracts/dpm_command_center.py` at 499 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 499 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 499/49 thresholds when the current largest file or
   function improves, and to keep `src/app/contracts/dpm_command_center.py` hotspot
