...
| Unit/contract coverage | Healthy | Current Advise workspace client-boundary branch focused validation passed with ruff, format check, touched-client mypy, 179 upstream-client/boundary tests, and refactor threshold gate at `max_source_file_lines=1041`; full local `make check` and `make ci` remain required before PR/merge |
