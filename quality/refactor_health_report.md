...
| Unit/contract coverage | Healthy | Current Workbench sandbox focused validation passed with 107 Workbench service and service-boundary tests and refactor-threshold proof at `max_source_file_lines=508`; prior full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 580 source files, OpenAPI smoke, and 1,273 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits Workbench sandbox orchestration into a focused module while preserving the existing Workbench service surface. The largest current source-file hotspot is now `src/app/services/advisor_brief_source.py` at 508 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 508 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 508/49 thresholds when the current largest file or
