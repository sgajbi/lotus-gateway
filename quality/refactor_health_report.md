...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits static portfolio workflow action specs and label policy into a focused helper while preserving portfolio workflow response behavior. The largest current source-file hotspot is `src/app/contracts/risk_workspace_examples.py` at 379 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 379 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 379/49 thresholds when the current largest file or
   function improves, and to keep `src/app/contracts/risk_workspace_examples.py` hotspot
