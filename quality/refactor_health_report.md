...
| Unit/contract coverage | In progress | Focused validation passed with 43 portfolio unit tests, mypy over the touched portfolio service/response modules, and refactor-threshold trials proving `max_source_file_lines=667` passes while `666` fails on `src/app/services/performance_workspace_horizon.py`; full local `make check` remains pending for this branch |
...
| Security audit | Governed | Current portfolio insights response boundary branch introduces no dependency, authentication, caller-context, product-error-detail, monetary-float policy, or upstream error-shape changes; full local `make ci` remains pending for this branch |
...
| Live canonical runtime | Pending | Latest merged live evidence passed after rebuilding `lotus-gateway` for PR #444; this branch still requires a fresh gateway rebuild and canonical Workbench validation before demo-ready sign-off |
| Observability evidence | Pending | Latest merged observability evidence captured 13/13 API checks at HTTP 200, 4/4 metric checks at HTTP 200, 14 log artifacts, and 5/5 observability screenshots at HTTP 200; this branch still requires a fresh companion observability pack |
