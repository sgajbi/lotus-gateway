...
| Unit/contract coverage | Healthy | Current portfolio workspace section contract extraction focused validation includes portfolio contract and contract-boundary tests plus a 316-line threshold pass proving the ceiling now lands on evidence-selected `src/app/services/foundation_service.py`; full local `make check` evidence remains required before commit |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves portfolio workspace section DTOs into `src/app/contracts/portfolio_workspace_sections.py`, keeping profile, reporting, rebalance, and operational-readiness contracts out of the response facade without moving the float-bearing performance summary. The evidence-selected largest current source-file hotspot is `src/app/services/foundation_service.py` at 316 lines |
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   ...
   function improves, and to keep `src/app/services/foundation_service.py` hotspot
