...
The current advisory protocol-boundary slice splits bank-demo proof, advisory copilot, advisory
workspace, advisor cockpit, advisory policy, and proposal protocol families into focused modules
while preserving the public `app.services.advisory_client_protocols` compatibility facade. Advisory
...
source-file ceiling blocker.
Advise proposal delivery client extraction now moves report-request, delivery-summary,
delivery-event, execution-handoff, execution-status, and execution-update upstream route
forwarding into `src/app/clients/advise_proposal_delivery_client.py` while preserving the public
`AdviseClient` surface. This reduces `src/app/clients/advise_proposal_client.py` below the prior
406-line ceiling and ratchets the blocking threshold from 406/49 to 405/49 because
`src/app/contracts/proposal_lifecycle.py` and `src/app/services/proposal_service.py` are now the
tied source-file ceiling blockers.
Proposal lifecycle contract and query-service extraction now moves summary, workflow, lineage, and
envelope DTOs into focused lifecycle contract modules, reducing
`src/app/contracts/proposal_lifecycle.py` from 405 to 21 lines, and moves workflow-events,
approvals, and lineage query orchestration into
`src/app/services/proposal_lifecycle_query_service.py`, reducing `src/app/services/proposal_service.py`
from 405 to 324 lines. This ratchets the blocking threshold from 405/49 to 404/49 because
`src/app/services/platform_capabilities_normalization.py` is now the sole source-file ceiling
blocker.
Platform capability feature and workflow flag extraction now moves source capability feature-key
and workflow-key interpretation into `src/app/services/platform_capabilities_feature_flags.py`,
reducing `src/app/services/platform_capabilities_normalization.py` from 404 to 192 lines. This
ratchets the blocking threshold from 404/49 to 402/49 because
`src/app/services/performance_workspace_contribution.py` and
`src/app/services/risk_workspace_service.py` are now the tied source-file ceiling blockers.
Performance contribution payload mapping now moves contribution level, row, position,
smoothing-evidence, and source-economics payload mapping into
`src/app/services/performance_workspace_contribution_payloads.py`, reducing
`src/app/services/performance_workspace_contribution.py` from 402 to 227 lines while preserving
Lotus Performance source-truth payload handling. The blocking threshold remains 402/49 because
`src/app/services/risk_workspace_service.py` is now the single source-file ceiling blocker.
Risk workspace response-loading extraction now moves summary, concentration, drawdown, rolling,
and rolling-Sharpe fallback upstream response loading into
`src/app/services/risk_workspace_response_loading.py`, reducing
`src/app/services/risk_workspace_service.py` from 402 to 222 lines while preserving cache,
correlation, unavailable envelope, and Lotus Risk source-truth behavior. This ratchets the
blocking threshold from 402/49 to 399/49 because
`src/app/services/dpm_proof_pack_service.py` is now the single source-file ceiling blocker.
DPM proof-pack supportability extraction now moves Manage proof-pack supportability derivation
into `src/app/services/dpm_proof_pack_supportability.py`, reducing
`src/app/services/dpm_proof_pack_service.py` from 399 to 315 lines while preserving proof-pack
generation, lookup, Markdown, report-input, AI-evidence-input, product-safe Manage error mapping,
and Lotus AI PM memo handoff behavior. This ratchets the blocking threshold from 399/49 to
398/49 because `src/app/contracts/advisor_brief.py` is now the single source-file ceiling blocker.
...
| Unit/contract coverage | Healthy | Current Foundation protocol-family extraction focused validation includes service-layer boundary tests, Foundation service tests, Foundation snapshot tests, and Foundation service factory tests plus a 328-line threshold pass proving the ceiling now lands on evidence-selected `src/app/services/risk_workspace_concentration.py`; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 686 source files, OpenAPI smoke, and 1,397 unit/contract tests |
...
| Security audit | Governed | Current performance workspace detail capability extraction introduces no dependency, authentication, caller-context, product-error-detail, upstream error-shape, data-mesh behavior, or runtime behavior changes; monetary-float governance remains governed by the existing blocking check; prior full local `make ci` passed `pip-audit` with no known vulnerabilities after the governed `PYSEC-2026-161` exception |
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves Foundation upstream protocol families into `src/app/services/foundation_client_protocols.py` while preserving the public `workspace_client_protocols` compatibility facade. The evidence-selected largest current source-file hotspot is `src/app/services/risk_workspace_concentration.py` at 328 lines |
...
1. Continue splitting large contract modules, service modules, observability modules, and remaining upstream clients
   around clear ownership boundaries. The next source-file hotspot is
   `src/app/services/risk_workspace_concentration.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 328 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 328/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/risk_workspace_concentration.py` hotspot
