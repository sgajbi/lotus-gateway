...
| Unit/contract coverage | Healthy | Current Lotus Core portfolio-query focused validation passed with 190 Lotus Core upstream-client, boundary, and factory tests, mypy over touched Lotus Core client modules, and refactor-threshold proof at `max_source_file_lines=480`; current full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 590 source files, OpenAPI smoke, and 1,281 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits Lotus Core portfolio query/read forwarding into a focused mixin while preserving the public concrete client surface. The largest current source-file hotspot is now `src/app/contracts/dpm_waves.py` at 480 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 480 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 480/49 thresholds when the current largest file or
