...
| Unit/contract coverage | Healthy | Current Advise proposal delivery client extraction focused validation includes Advise client-boundary, refactor-threshold, quality-baseline artifact, and agent-quality evidence tests; full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 618 source files, OpenAPI smoke, and 1,309 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits Advise proposal delivery/report/execution upstream route forwarding into a focused helper mixin while preserving public `AdviseClient` behavior. The largest current source-file hotspots are now `src/app/contracts/proposal_lifecycle.py` and `src/app/services/proposal_service.py` at 405 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/contracts/proposal_lifecycle.py` and `src/app/services/proposal_service.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 405 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 405/49 thresholds when the current largest file or
   function improves, and to keep `src/app/contracts/proposal_lifecycle.py` hotspot
