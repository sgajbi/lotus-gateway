...
| --- | --- | --- |
| Branch hygiene | Healthy | Current portfolio book response assembly branch was created from clean `main` after PR #393; final remote/local cleanup remains a post-merge gate |
| Unit/contract coverage | Healthy | Current portfolio book response assembly branch `make check` passed with ruff, format check, monetary-float guard, refactor threshold gate, mypy over 475 source files, Workbench/OpenAPI contract smoke, and 1,115 unit/contract tests |
| Integration coverage | Healthy | Current portfolio book response assembly branch `make ci` passed with 207 integration tests |
| Total coverage | Healthy | Current portfolio book response assembly branch `make ci` passed with 1,322 coverage tests and 94.14% total coverage, above the 84% floor |
| Security audit | Governed | Current portfolio book response assembly branch `pip-audit` found no known vulnerabilities after the governed `PYSEC-2026-161` exception; no new dependency or monetary-float conversion was introduced |
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; recent response/request-context/parser/mapper/contract slices reduce `portfolio_service.py` to the 1,799-line source-file threshold; `advisor_brief_service.py` is the next-largest source file at 1,609 physical lines, and several service files remain above 1,000 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 1,799 physical lines,
