...
| Unit/contract coverage | Healthy | Current performance attribution supportability contract extraction focused validation includes performance attribution contract tests, refactor-threshold, quality-baseline artifact, and agent quality evidence tests, with refactor-threshold proof at `max_source_file_lines=438`; full local `make check` passed with 1,294 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits performance attribution supportability contracts into a focused module while preserving public attribution compatibility imports. The largest current source-file hotspot is now `src/app/services/advisor_brief_service.py` at 438 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 438 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 438/49 thresholds when the current largest file or
