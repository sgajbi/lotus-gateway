...
The current platform capabilities workspace-descriptor slice moves workspace descriptor policy and
...
blocking source-file threshold is ratcheted from 488 to 481 lines.
...
| Unit/contract coverage | Healthy | Current platform capabilities workspace-descriptor focused validation passed with platform capability shell, normalization, service, and service-layer boundary tests, mypy over touched capability modules, and refactor-threshold proof at `max_source_file_lines=481`; current full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 589 source files, OpenAPI smoke, and 1,280 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits platform capability workspace descriptor policy/state mapping into a focused module while preserving the public shell import surface. The largest current source-file hotspot is now `src/app/clients/lotus_core_query_client.py` at 481 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 481 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 481/49 thresholds when the current largest file or
   function improves, and to keep `src/app/clients/lotus_core_query_client.py` hotspot
