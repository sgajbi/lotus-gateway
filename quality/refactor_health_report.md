...
| Unit/contract coverage | Healthy | Current portfolio holdings-orchestration focused validation passed with 101 portfolio/service-boundary/quality tests and refactor-threshold proof at `max_source_file_lines=575`; full local `make check` passed with workflow governance, mypy over 567 source files, OpenAPI smoke, and 1,250 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits portfolio holdings orchestration into a focused mixin while preserving the public service surface. The largest current source-file hotspot is now `src/app/observability/analytics_ui.py` at 575 script-counted lines |
