...
| Unit/contract coverage | Healthy | Current Foundation workspace source-loading extraction focused validation includes Foundation service, Foundation contract, and service-boundary tests plus a 316-line threshold pass proving the ceiling now lands on evidence-selected `src/app/services/platform_capabilities_workspace_descriptors.py`; full local `make check` evidence remains required before commit |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves Foundation workspace source loading into `src/app/services/foundation_workspace_sources.py`, keeping core/optional source fan-out and performance reference-date fallback out of the main Foundation response service. The evidence-selected largest current source-file hotspot is `src/app/services/platform_capabilities_workspace_descriptors.py` at 316 lines |
...
1. Continue splitting large contract modules, service modules, observability modules, and remaining upstream clients
   ...
   `src/app/services/platform_capabilities_workspace_descriptors.py`.
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   ...
   function improves, and to keep `src/app/services/platform_capabilities_workspace_descriptors.py` hotspot
