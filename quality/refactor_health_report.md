...
| Unit/contract coverage | Healthy | Current Foundation core market-value extraction focused validation includes Foundation core snapshot tests, refactor-threshold, mypy, and agent quality evidence tests; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 655 source files, OpenAPI smoke, and 1,370 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves Foundation core market-value key selection and quantized-money parsing into a focused helper while preserving the public mapper method. The largest current source-file hotspot is `src/app/services/dpm_pm_operating_quality_service.py` at 356 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/services/dpm_pm_operating_quality_service.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 356 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 356/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/dpm_pm_operating_quality_service.py` hotspot
