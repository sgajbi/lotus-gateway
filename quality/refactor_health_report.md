...
| Unit/contract coverage | Healthy | Current CI enforcement focused validation passed with 26 agent quality evidence, quality-artifact, workflow-action runtime, and refactor-threshold tests and refactor-threshold proof at `max_source_file_lines=521`; current full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 578 source files, OpenAPI smoke, and 1,271 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits risk rolling window mapping into a focused module while preserving the existing rolling response mapper. The largest current source-file hotspot is now `src/app/services/dpm_command_center_service.py` at 521 script-counted lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 521 script-counted lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 521/49 thresholds when the current largest file or
