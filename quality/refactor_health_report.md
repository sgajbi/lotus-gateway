...
| Unit/contract coverage | Healthy | Current risk workspace request-payload focused validation includes risk workspace request, service, cache, attribution, refactor-threshold, quality-baseline artifact, and agent quality evidence tests, with refactor-threshold proof at `max_source_file_lines=444`; full local `make check` validation is required before merge |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits Lotus Risk request payload construction into a focused payload module while preserving public `risk_workspace_requests` compatibility. The largest current source-file hotspot is now `src/app/services/advisor_brief_narrative.py` at 444 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/services/advisor_brief_narrative.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 444 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 444/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/advisor_brief_narrative.py` hotspot
