...
| Unit/contract coverage | Healthy | Current Foundation catalog contract extraction focused validation includes Foundation contract tests, Foundation service tests, Foundation core snapshot tests, contract-module boundary tests, and agent quality evidence checks; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 658 source files, OpenAPI smoke, and 1,373 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves Foundation catalog contracts into a focused module while preserving the public `app.contracts.foundation` facade. The largest current source-file hotspot is `src/app/services/performance_workspace_service.py` at 354 lines |
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   ...
   function improves, and to keep `src/app/services/performance_workspace_service.py` hotspot
