...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves the risk drawdown payload schema example into the focused drawdown examples module while preserving the public drawdown payload contract. The largest current source-file hotspot is `src/app/services/advisor_brief_source.py` at 366 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/services/advisor_brief_source.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 366 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 366/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/advisor_brief_source.py` hotspot
