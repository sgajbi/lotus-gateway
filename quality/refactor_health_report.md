...
| Total coverage | Healthy | Full local `make ci` passed with 1,376 combined coverage tests and 94.22% total coverage |
| Security audit | Governed | Current foundation core-snapshot mapper branch introduces no dependency, authentication, caller-context, monetary-float, or error-detail policy changes; prior full local `make ci` `pip-audit` reported no known vulnerabilities after the governed `PYSEC-2026-161` exception |
