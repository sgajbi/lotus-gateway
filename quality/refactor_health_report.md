...
| --- | --- | --- |
| Branch hygiene | Healthy | Current proposal service-boundary branch was created from clean `main`; no open PRs and no unmerged remote branches were present at slice start; final remote/local cleanup remains a post-merge gate |
| Unit/contract coverage | Healthy | Focused proposal service-boundary validation passed with 42 proposal-service, service-boundary, and threshold tests and refactor-threshold trials proving `max_source_file_lines=646` passes while `645` fails on `src/app/contracts/advisor_brief.py`; full local `make check` passed with 1,226 unit/contract tests |
| Integration coverage | Healthy | Full local `make ci` passed with migration contract smoke and 209 integration tests |
| Total coverage | Healthy | Full local `make ci` passed with 1,435 combined coverage tests and 94.32% total coverage |
| Security audit | Governed | Current performance/reporting contract-boundary branch introduces no dependency, authentication, caller-context, product-error-detail, or upstream error-shape changes; the governed monetary-float allowlist was refreshed for moved performance response float annotations and the monetary-float guard passes with 159 allowlisted findings; full local `make ci` security audit found no known vulnerabilities after the governed `PYSEC-2026-161` exception |
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; recent response/request-context/parser/mapper/contract slices lower `portfolio_service.py` to 589 script-counted lines, `performance_workspace_horizon.py` to 441 script-counted lines, `workbench_service.py` to 562 script-counted lines, `lotus_analytics_client.py` to 623 script-counted lines, `advisor_brief_source.py` to 508 script-counted lines, `risk_workspace_service.py` to 632 script-counted lines, `performance_workspace_evidence.py` to 461 script-counted lines, `workbench.py` to 47 script-counted lines, `performance_workspace_service.py` to 639 lines, `advisor_brief_service.py` to 435 script-counted lines, `advisory_client_protocols.py` to 630 script-counted lines, `advisor_brief_client_protocols.py` to 63 script-counted lines, `proposal_service.py` to 520 script-counted lines, `proposal_transition_service.py` to 156 script-counted lines, `dpm_command_center_service.py` to 521 lines, `dpm_command_center_exception_summary.py` to 187 lines, `dpm_wave_service.py` to 479 script-counted lines, `dpm_wave_campaign_definitions.py` to 244 script-counted lines, `advise_client.py` to 220 script-counted lines, `advise_proposal_client.py` to 536 script-counted lines, `dpm_client.py` to 452 script-counted lines, `src/app/contracts/dpm_command_center.py` to 593 script-counted lines, `src/app/contracts/proposals.py` to 431 script-counted lines, `src/app/contracts/advisor_brief.py` to 646 script-counted lines, `src/app/contracts/portfolio.py` to 281 script-counted lines, `src/app/contracts/portfolio_workspace.py` to 503 script-counted lines, `src/app/services/foundation_service.py` to 618 script-counted lines, `src/app/contracts/performance_workspace.py` to a 79-line compatibility facade, `src/app/contracts/reporting_batches.py` to a 75-line compatibility facade, `src/app/contracts/risk_workspace.py` to 312 script-counted lines, `src/app/contracts/risk_workspace_examples.py` to 379 script-counted lines, `src/app/router_registry.py` to 632 script-counted lines, and `src/app/contracts/reporting_query.py` to a 41-line compatibility facade; `src/app/contracts/advisor_brief.py` is now the largest current source-file hotspot at 646 lines |
| Live canonical runtime | Healthy | Rebuilt `lotus-gateway` from this branch, restored the canonical `lotus-advise` runtime after an initial missing-container failure, reseeded `PB_SG_GLOBAL_BAL_001`, and passed `npm run live:validate` through canonical Workbench/Gateway/downstream paths. Machine-readable evidence at `lotus-workbench/output/playwright/live-canonical/live-validation-summary.json` was generated at `2026-06-18T14:46:17.019Z` with 94 API checks, 29 screenshots, 25 ready panel classifications, 2 calculation checks, 28 supportability checks, 10 workflow-pack checks, and 12 advisory journey checks |
| Observability evidence | Healthy | Companion pack `lotus-workbench/output/observability-live/20260618-202754/observability-evidence-manifest.json` captured canonical DNS, container inventory, readiness/API samples, metrics, logs, and screenshots; 13/13 API checks returned HTTP 200, 4/4 metric checks returned HTTP 200, 14 log artifacts were captured, and Gateway logs show structured request evidence |
