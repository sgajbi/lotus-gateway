...
| Unit/contract coverage | Healthy | Current portfolio transaction activity-summary focused validation passed with transaction-summary tests, mypy over touched transaction modules, and refactor-threshold proof at `max_source_file_lines=503`; current full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 582 source files, OpenAPI smoke, and 1,275 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits portfolio transaction activity summary into a focused module while keeping monetary amount helpers in their existing governed allowlisted module. The largest current source-file hotspot is now `src/app/contracts/portfolio_workspace.py` at 503 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 503 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 503/49 thresholds when the current largest file or
