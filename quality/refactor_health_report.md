...
| Unit/contract coverage | Healthy | Current DPM command-center contract focused validation passed with DPM command-center contract and contract-module-boundary tests, mypy over touched contract modules, and refactor-threshold proof at `max_source_file_lines=497`; current full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 585 source files, OpenAPI smoke, and 1,277 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits DPM command-center core and outcome-review contracts into focused contract modules while preserving compatibility imports through the command-center facade. The largest current source-file hotspot is now `src/app/services/performance_workspace_service.py` at 497 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 497 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 497/49 thresholds when the current largest file or
