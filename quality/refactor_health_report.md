...
The current Lotus Core simulation-session client boundary slice moves simulation-session create,
...
| Unit/contract coverage | Healthy | Current Lotus Core simulation-session client focused validation passed with 232 Lotus Core client-boundary, upstream-client, Workbench router, quality-artifact, and refactor-threshold tests and refactor-threshold proof at `max_source_file_lines=525`; current full local `make check` passed with workflow governance, refactor thresholds, mypy over 576 source files, OpenAPI smoke, and 1,263 unit/contract tests |
