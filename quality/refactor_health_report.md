...
| Unit/contract coverage | Healthy | Current risk concentration supportability extraction focused validation includes risk workspace service tests, service-layer boundary tests, and a 326-line threshold pass proving the ceiling now lands on evidence-selected `src/app/contracts/platform_capabilities.py`; full local `make check` evidence remains required before commit |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves concentration block extraction and supportability narration into `src/app/services/risk_workspace_concentration_supportability.py` while preserving the public concentration response mapper behavior. The evidence-selected largest current source-file hotspot is `src/app/contracts/platform_capabilities.py` at 326 lines |
...
1. Continue splitting large contract modules, service modules, observability modules, and remaining upstream clients
   ...
   `src/app/contracts/platform_capabilities.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 326 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 326/49 thresholds when the current largest file or
   function improves, and to keep `src/app/contracts/platform_capabilities.py` hotspot
