...
| Unit/contract coverage | Healthy | Current DPM wave campaign-workflow focused validation passed with 22 DPM wave contract and contract-boundary tests, mypy over touched DPM wave contract modules, and refactor-threshold proof at `max_source_file_lines=479`; current full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 591 source files, OpenAPI smoke, and 1,282 unit/contract tests |
...
1. Continue splitting large service modules and remaining upstream clients around clear service
   and route-family boundaries. The next source-file hotspots are
   `src/app/services/risk_workspace_drawdown.py` and `src/app/services/dpm_wave_service.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 479 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 479/49 thresholds when the current largest file or
