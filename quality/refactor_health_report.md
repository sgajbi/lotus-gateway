...
| Unit/contract coverage | Healthy | Current performance horizon standard-window extraction focused validation includes horizon helper tests, refactor-threshold, quality-baseline artifact, and agent quality evidence tests, with refactor-threshold proof at `max_source_file_lines=440`; full local `make check` passed with 1,293 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits performance standard-horizon fetch and merge helpers into a focused module while preserving public horizon helper behavior. The largest current source-file hotspot is now `src/app/contracts/performance_attribution.py` at 440 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 440 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 440/49 thresholds when the current largest file or
