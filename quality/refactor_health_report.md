...
| Unit/contract coverage | Healthy | Current Advise proposal narrative client extraction focused validation includes Advise client boundary tests, upstream proposal route tests, and route-coverage contract tests plus a 329-line threshold pass proving the ceiling now lands on evidence-selected `src/app/services/workspace_client_protocols.py`; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 685 source files, OpenAPI smoke, and 1,395 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves Advise proposal narrative routes into `src/app/clients/advise_proposal_narrative_client.py` while preserving the public `AdviseClient` facade. The evidence-selected largest current source-file hotspot is `src/app/services/workspace_client_protocols.py` at 329 lines |
...
1. Continue splitting large contract modules, service modules, observability modules, and remaining upstream clients
   ...
   `src/app/services/workspace_client_protocols.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 329 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 329/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/workspace_client_protocols.py` hotspot
