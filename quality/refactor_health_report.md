...
| Unit/contract coverage | Healthy | Current platform capability bootstrap contract extraction focused validation includes platform capability contract, normalization, shell, and contract-boundary tests plus a 325-line threshold pass proving the ceiling now lands on evidence-selected `src/app/services/platform_capabilities_service.py`; full local `make check` evidence remains required before commit |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves shell bootstrap DTOs into `src/app/contracts/platform_capabilities_bootstrap.py` while preserving the public `platform_capabilities` compatibility facade. The evidence-selected largest current source-file hotspot is `src/app/services/platform_capabilities_service.py` at 325 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 325 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 325/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/platform_capabilities_service.py` hotspot
