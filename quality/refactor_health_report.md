...
| Unit/contract coverage | Healthy | Current DPM outcome-review narrative focused validation passed with 68 DPM command-center service and service-boundary tests and refactor-threshold proof at `max_source_file_lines=520`; current full local `make check` passed with workflow governance, refactor thresholds, agent quality evidence, mypy over 579 source files, OpenAPI smoke, and 1,272 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits DPM outcome-review narrative handoff into a focused module while preserving the existing command-center service surface. The largest current source-file hotspot is now `src/app/services/proposal_service.py` at 520 script-counted lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 520 script-counted lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 520/49 thresholds when the current largest file or
