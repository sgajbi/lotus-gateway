...
| Unit/contract coverage | Healthy | Current DPM exception-summary contract extraction focused validation includes DPM command-center contract shape, contract-boundary, refactor-threshold, mypy, and agent quality evidence tests; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 654 source files, OpenAPI smoke, and 1,370 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves DPM exception-summary request/response DTOs into a focused contract module while preserving public facade and compatibility re-export paths. The largest current source-file hotspot is `src/app/services/foundation_core_snapshot.py` at 357 lines |
