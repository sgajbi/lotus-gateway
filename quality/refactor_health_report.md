...
| Unit/contract coverage | Healthy | Current risk concentration example extraction focused validation includes risk concentration contract tests, contract-module boundary tests, Workbench OpenAPI contract tests, a 346-line threshold failure trial proving the 347-line ceiling remains, and agent quality evidence checks; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 668 source files, OpenAPI smoke, and 1,379 unit/contract tests |
...
| Security audit | Governed | Current risk concentration example extraction introduces no dependency, authentication, caller-context, product-error-detail, upstream error-shape, data-mesh behavior, or runtime behavior changes; monetary-float governance remains at 150 findings with 152 allowlisted; prior full local `make ci` passed `pip-audit` with no known vulnerabilities after the governed `PYSEC-2026-161` exception |
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves the risk concentration schema example into a focused example module while preserving public contract exports and OpenAPI example shape. The largest current source-file hotspot is `src/app/services/portfolio_holdings_service.py` at 347 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/services/portfolio_holdings_service.py`.
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   ...
   function improves, and to keep `src/app/services/portfolio_holdings_service.py` hotspot
