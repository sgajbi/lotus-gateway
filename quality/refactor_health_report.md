...
| Unit/contract coverage | Healthy | Current DPM router-group split focused validation includes router-registry and DPM command-center, construction, proof-pack, and wave contract tests, a 346-line threshold failure trial proving the new 347-line ceiling, and agent quality evidence checks; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 667 source files, OpenAPI smoke, and 1,378 unit/contract tests |
...
| Security audit | Governed | Current DPM router-group split introduces no dependency, authentication, caller-context, product-error-detail, upstream error-shape, data-mesh behavior, or runtime behavior changes; monetary-float governance remains at 150 findings with 152 allowlisted; prior full local `make ci` passed `pip-audit` with no known vulnerabilities after the governed `PYSEC-2026-161` exception |
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves DPM command-center, campaign, proof/construction, and wave router aggregation into focused modules while preserving the public `app.router_groups.dpm` exports. The largest current source-file hotspots are `src/app/contracts/risk_workspace_concentration.py` and `src/app/services/portfolio_holdings_service.py` at 347 lines |
...
1. Continue splitting large contract modules, service modules, and remaining upstream clients
   ...
   `src/app/contracts/risk_workspace_concentration.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 347 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 347/49 thresholds when the current largest file or
   function improves, and to keep `src/app/contracts/risk_workspace_concentration.py` hotspot
