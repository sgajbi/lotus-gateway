...
| --- | --- | --- |
| Branch hygiene | Healthy | Current performance workspace boundary branch was created from clean `main`; no open PRs and no unmerged remote branches were present at slice start; final remote/local cleanup remains a post-merge gate |
| Unit/contract coverage | Healthy | Focused performance workspace boundary validation passed with 69 targeted detail-view/service-boundary/threshold tests; full local `make check` passed with 1,231 unit/contract tests, and refactor-threshold trials prove `max_source_file_lines=632` passes while `631` fails on `src/app/router_registry.py` and `src/app/services/risk_workspace_service.py` |
| Integration coverage | Healthy | Full local `make ci` passed with migration contract smoke and 209 integration tests |
| Total coverage | Healthy | Full local `make ci` passed with 1,440 combined coverage tests and 94.33% total coverage |
| Security audit | Governed | Current performance workspace boundary branch introduces no dependency, authentication, caller-context, product-error-detail, upstream error-shape, or monetary-float conversion changes; full local `make ci` passed `pip-audit` with no known vulnerabilities after the governed `PYSEC-2026-161` exception |
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch extracts performance workspace detail-view orchestration into `src/app/services/performance_workspace_detail_views.py` and lowers the blocking source-file ceiling to 632 lines. The largest current source-file hotspots are now `src/app/router_registry.py` and `src/app/services/risk_workspace_service.py` at 632 script-counted lines |
| Live canonical runtime | Healthy for latest clean-main proof; not rerun for this branch | Latest clean-main canonical proof passed `npm run live:validate` through canonical Workbench/Gateway/downstream paths for `PB_SG_GLOBAL_BAL_001`. Machine-readable evidence at `lotus-workbench/output/playwright/live-canonical/live-validation-summary.json` was generated at `2026-06-18T16:04:19.273Z` with 94 API checks, 29 screenshots, 25 ready panel classifications, 2 calculation checks, and no missing or non-ready panels. This branch moves internal performance workspace detail-view orchestration behind a helper module and does not change runtime routing, downstream clients, response fields, dependencies, or caller-context/security handling |
| Observability evidence | Healthy for latest clean-main proof; not rerun for this branch | Latest clean-main companion pack `lotus-workbench/output/observability-live/20260619-000802/observability-evidence-manifest.json` captured canonical DNS, container inventory, readiness/API samples, metrics, logs, and screenshots; 13/13 API checks returned HTTP 200, 4/4 metric checks returned HTTP 200, 14 log artifacts were captured, Prometheus reported 14/14 targets up, and Gateway logs show structured correlation/request/trace evidence |
