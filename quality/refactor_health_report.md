...
| Unit/contract coverage | Healthy | Current proposal generation contract extraction focused validation includes proposal contract, contract-module boundary, refactor-threshold, quality-baseline artifact, and agent quality evidence tests, with refactor-threshold proof at `max_source_file_lines=429`; full local `make check` passed with 1,300 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits proposal generation contracts into a focused module while preserving public proposal facade behavior. The largest current source-file hotspot is now `src/app/services/advisor_brief_source.py` at 429 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 429 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 429/49 thresholds when the current largest file or
