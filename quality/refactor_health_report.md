...
| Unit/contract coverage | Healthy | Current performance workspace detail capability extraction focused validation includes performance workspace capability behavior tests and service-layer boundary tests plus a 334-line threshold pass proving the ceiling now lands on evidence-selected `src/app/services/foundation_core_snapshot.py`; full local `make check` passed with Ruff, format, monetary-float guard, workflow governance, refactor thresholds, agent quality evidence, folder guides, mypy over 681 source files, OpenAPI smoke, and 1,391 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch moves performance workspace detail capability policy into `src/app/services/performance_workspace_detail_capabilities.py` and shared module capability construction into `src/app/services/performance_workspace_module_capability.py` while preserving the public performance workspace capability facade. The evidence-selected largest current source-file hotspot is `src/app/services/foundation_core_snapshot.py` at 334 lines |
...
1. Continue splitting large contract modules, service modules, observability modules, and remaining upstream clients
   ...
   `src/app/services/foundation_core_snapshot.py`.
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 334 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 334/49 thresholds when the current largest file or
   function improves, and to keep `src/app/services/foundation_core_snapshot.py` hotspot
