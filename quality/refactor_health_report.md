...
| Unit/contract coverage | Healthy | Current portfolio transaction income-summary focused validation passed with 21 portfolio transaction summary, refactor-threshold, quality-baseline artifact, and agent quality evidence tests, touched-module mypy, and refactor-threshold proof at `max_source_file_lines=458`; full local `make check` passed with monetary-float governance at 152 findings/152 allowlisted, workflow governance, refactor thresholds, agent quality evidence, mypy over 598 source files, OpenAPI smoke, and 1,288 unit/contract tests |
...
| Modularity | Improving, incomplete | Longest-function baseline remains 49 lines; current branch splits portfolio transaction income summary construction and shared amount helpers into focused modules while preserving public import compatibility. The largest current source-file hotspot is now `src/app/services/workspace_client_protocols.py` at 458 lines |
...
2. Blocking refactor threshold gate now enforces:
   - no Python source file under `src/app` above 458 lines,
...
3. The blocking agent quality evidence gate runs `scripts/check_agent_quality_evidence.py` to
   make future agent slices ratchet the 458/49 thresholds when the current largest file or
