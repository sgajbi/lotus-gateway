...
5. agent quality evidence governance through `scripts/check_agent_quality_evidence.py`, which
   keeps the executable 355/49 ratchet, the current largest hotspot
   `src/app/contracts/reporting.py`, and durable scorecard/context guidance in
   sync,
6. `mypy` over `src`,
...
7. Quality Baseline and `make lint` now run the blocking agent quality evidence gate through
   `scripts/check_agent_quality_evidence.py`, proving the executable 355/49 ratchet still matches
   current source truth and that durable scorecard/context guidance names
   `src/app/contracts/reporting.py` as the current 355-line hotspot.
