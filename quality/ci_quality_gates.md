...
5. agent quality evidence governance through `scripts/check_agent_quality_evidence.py`, which
   keeps the executable 331/49 ratchet, the current evidence-selected largest hotspot
   `src/app/contracts/domain_products.py`, and durable scorecard/context
   guidance in sync,
6. `mypy` over `src`,
...
7. Quality Baseline and `make lint` now run the blocking agent quality evidence gate through
   `scripts/check_agent_quality_evidence.py`, proving the executable 331/49 ratchet still matches
   current source truth and that durable scorecard/context guidance names
   `src/app/contracts/domain_products.py` as the current evidence-selected
   331-line hotspot after foundation core snapshot section extraction.
