...
5. agent quality evidence governance through `scripts/check_agent_quality_evidence.py`, which
   keeps the executable 520/49 ratchet, the current largest hotspot
   `src/app/services/proposal_service.py`, and durable scorecard/context guidance in
   sync,
6. `mypy` over `src`,
...
2. Current enforced source-file threshold: no Python source file under `src/app` above 520
...
4. `python scripts/check_refactor_quality_thresholds.py`: passed with
   `max_source_file_lines=520` and `max_function_lines=49`.
...
7. Quality Baseline and `make lint` now run the blocking agent quality evidence gate through
   `scripts/check_agent_quality_evidence.py`, proving the executable 520/49 ratchet still matches
   current source truth and that durable scorecard/context guidance names
   `src/app/services/proposal_service.py` as the 520-line hotspot.
