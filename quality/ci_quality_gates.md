...
5. agent quality evidence governance through `scripts/check_agent_quality_evidence.py`, which
   keeps the executable 324/49 ratchet, the current evidence-selected largest hotspot
   `src/app/services/proposal_service.py`, and durable scorecard/context
   guidance in sync,
6. `mypy` over `src`,
...
7. Quality Baseline and `make lint` now run the blocking agent quality evidence gate through
   `scripts/check_agent_quality_evidence.py`, proving the executable 324/49 ratchet still matches
   current source truth and that durable scorecard/context guidance names
   `src/app/services/proposal_service.py` as the current evidence-selected
   324-line hotspot after platform capability service cleanup.
