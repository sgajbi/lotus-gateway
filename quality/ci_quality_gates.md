...
4. workflow action-runtime governance for platform-baseline GitHub Actions majors and the Node 24
   JavaScript action opt-in,
5. `mypy` over `src`,
...
15. Current Node 24 workflow-runtime branch adds
    `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24: "true"` to governed Gateway workflows and extends
    `scripts/check_workflow_action_runtime.py` so `make lint` blocks workflows that use governed
    GitHub JavaScript actions without the opt-in. Local `make check` passed with 1,111
    unit/contract tests, and local `make ci` passed with 207 integration tests, 1,318 coverage
    tests, 94.13% total coverage, and no known vulnerabilities after the governed
    `PYSEC-2026-161` exception.
