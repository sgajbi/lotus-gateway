...
5. agent quality evidence governance through `scripts/check_agent_quality_evidence.py`, which
   keeps the executable 335/49 ratchet, the current evidence-selected largest hotspot
   `src/app/contracts/risk_workspace_rolling.py`, and durable scorecard/context guidance in sync,
6. `mypy` over `src`,
...
7. Quality Baseline and `make lint` now run the blocking agent quality evidence gate through
   `scripts/check_agent_quality_evidence.py`, proving the executable 335/49 ratchet still matches
   current source truth and that durable scorecard/context guidance names
   `src/app/contracts/risk_workspace_rolling.py` as the current evidence-selected 335-line hotspot.
