...
Occurrence ordinals are assigned by measured location order but are not themselves part of the
absolute source coordinates, so unrelated line shifts or body edits do not invalidate every
fingerprint. A removed clone also creates a stale-fingerprint failure until a reviewed
`--update-baseline` banks the
