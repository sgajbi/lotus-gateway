...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current performance attribution contract branch `make check` passed with ruff, format check, monetary-float guard, mypy over 470 source files, Workbench/OpenAPI contract smoke, and 1,058 unit/contract tests; `make ci` passed with 207 integration tests, 1,265 coverage tests, 94.05% total coverage, and dependency audit | Keep Docker parity blocking in PR Merge Gate |
| Coverage | 4/5 | 94.05% total coverage, above the 84% floor | Add targeted middleware/security/error tests |
| Modularity | 4/5 | Current branch state preserves the 49-line longest-function baseline; recent slices extracted transaction request context, transaction page-context defaults, transaction client-kwargs mapping, performance workspace response assembly, portfolio workspace response assembly, portfolio workspace performance parsing, portfolio workspace rebalance parsing, portfolio source-readiness parsing, portfolio transaction summary mapping, portfolio workflow mapping, portfolio workflow contracts, portfolio transaction contracts, portfolio performance snapshot contracts, portfolio income/activity contracts, portfolio holdings/book contracts, risk drawdown contracts, reporting batch contracts, reporting query contracts, risk concentration contracts, risk rolling contracts, risk attribution contracts, performance contribution contracts, and performance attribution contracts; `portfolio_service.py` is now 1,970 measured lines, `performance_workspace.py` is now 1,101 measured lines, `portfolio.py` is now 911 measured lines, `risk_workspace.py` is now 678 measured lines, and `reporting.py` is now 532 measured lines | Continue extraction slices and reduce remaining largest-file pressure |
...
| Security | 4/5 | Current performance attribution contract branch `pip-audit` passed with the governed FastAPI/Starlette exception; monetary-float guard passed after refreshing moved performance attribution-field allowlist paths | Triage bandit and sensitive-data handling checks |
...
versus the current performance attribution contract branch after PRs #349, #350, #351, #352,
#353, #354, #355, #356, #357, #358, #359, #360, #361, #362, #363, #364, #365, #366, #367,
#368, #369, #370, and #371.

| Measure | Prior scorecard | Current branch | Result |
| --- | ---: | ---: | --- |
| Counted files under `src`, `tests`, `docs`, `wiki`, `.github`, `scripts` | 1,265 | 1,375 | Added focused modules, tests, and quality evidence |
| Tracked `src/app` Python files | 447 | 470 | Added focused portfolio/performance response helpers and workspace/readiness/transaction summary/workflow mapper and contract modules plus risk drawdown, reporting batch, reporting query, risk concentration, risk rolling, risk attribution, performance contribution, and performance attribution contracts |
...
| Largest source file | 2,968 lines | 1,970 lines | Improved; `src/app/services/portfolio_service.py` remains the largest residual hotspot after reducing `performance_workspace.py` to 1,101 lines, `portfolio.py` to 911 lines, `risk_workspace.py` to 678 lines, and `reporting.py` to 532 lines |
| `performance_workspace.py` | 1,539 lines | 1,101 lines | Improved through performance contribution and attribution contract extraction |
| OpenAPI operations with missing summary/description/tags/errors | 0 | 0 | Preserved |
| Local unit/contract tests | 996 | 1,058 | Added focused response/request boundary, workspace parser, source-readiness parser, transaction summary mapper, workflow mapper, workflow contract, transaction contract, performance snapshot contract, income/activity contract, holdings contract, risk drawdown contract, reporting batch contract, reporting query contract, risk concentration contract, risk rolling contract, risk attribution contract, performance contribution contract, and performance attribution contract tests |
| Local coverage tests | 1,203 | 1,265 | Added focused coverage while preserving total coverage |
