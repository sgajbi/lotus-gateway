...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Bank-Buyable observability controls branch keeps the blocking refactor threshold gate green; `make check` passed with ruff, format check, monetary-float guard, the refactor threshold gate, mypy over 472 source files, Workbench/OpenAPI contract smoke, and 1,077 unit/contract tests; `make ci` passed with 207 integration tests, 1,284 coverage tests, 94.07% total coverage, and dependency audit | Keep Docker parity blocking in PR Merge Gate |
...
| Observability | 3/5 | Health/readiness/metrics/correlation/audit are present; RFC-0108 fan-out and selected analytics audit posture remain implementation-backed; analytics UI validators now enforce separate fan-out log and audit event families | Promote broader structured log and metric label rules into blocking checks |
...
| Documentation | 4/5 | Baseline, scorecard, health report, and observability docs are refreshed to current branch metrics after the current Bank-Buyable observability controls slice | Keep wiki synced and add diagrams over time |
