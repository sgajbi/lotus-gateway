...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Foundation catalog-payload boundary branch splits portfolio catalog item parsing into a focused helper while preserving the `FoundationService` surface. Local `make check` passed with workflow governance, mypy over 562 source files, OpenAPI smoke, and 1,245 unit/contract tests. The quality-baseline workflow now enforces the 610/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Run GitHub gates before merge |
...
| Largest source file | 2,968 lines | 610 script-counted lines | Improved and protected by the 610-line blocking threshold; `src/app/clients/lotus_core_query_client.py` is the single largest residual hotspot after splitting Foundation catalog payload parsing |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 610/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 610-line blocking ceiling.
