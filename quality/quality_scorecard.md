...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current performance trend service-boundary branch ratchets the blocking refactor threshold from 1,206 to 1,098 script-counted lines after reducing `performance_workspace_service.py` below the prior ceiling. Focused ruff, touched-module mypy, performance workspace trend/horizon/attribution/context/control tests, and the trial refactor threshold gate passed; full local `make check` and `make ci` evidence remains pending for this branch | Keep Docker parity blocking in PR Merge Gate |
...
| Security | 4/5 | Current performance trend service-boundary branch introduces no dependency, authentication, caller-context, monetary-float, or product-error-detail policy changes; full local `make ci` security audit evidence remains pending for this branch | Triage bandit and sensitive-data handling checks |
