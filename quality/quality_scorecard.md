...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 325-line source-file ceiling. This slice moves shell bootstrap DTOs into `src/app/contracts/platform_capabilities_bootstrap.py` while preserving the public `platform_capabilities` compatibility facade. The evidence-selected largest residual hotspot is now `src/app/services/platform_capabilities_service.py` at 325 lines. | Continue with the next cohesive 325-line hotspot slice |
...
| Largest source file | 2,968 lines | 325 lines | Improved and protected by the 325-line blocking threshold; `src/app/services/platform_capabilities_service.py` is the evidence-selected largest residual hotspot after extracting platform capability bootstrap contracts |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 325/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 325-line blocking ceiling.
