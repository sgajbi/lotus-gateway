...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 324-line source-file ceiling. This slice removes legacy private pass-through helpers from `src/app/services/platform_capabilities_service.py`, leaving feature, workflow, and module-health decisions in their owning normalization modules. The evidence-selected largest residual hotspot is now `src/app/services/proposal_service.py` at 324 lines. | Continue with the next cohesive 324-line hotspot slice |
...
| Largest source file | 2,968 lines | 324 lines | Improved and protected by the 324-line blocking threshold; `src/app/services/proposal_service.py` is the evidence-selected largest residual hotspot after removing legacy platform capability service pass-through helpers |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 324/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 324-line blocking ceiling.
