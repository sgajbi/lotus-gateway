...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current performance/reporting contract-boundary branch ratchets the blocking refactor threshold from 662 to 658 script-counted lines after splitting performance workspace common/summary/details contracts and report-batch examples, materialization, worker, and scheduler contracts behind existing compatibility facades. Focused validation passed with 18 reporting/performance contract and boundary tests, and refactor-threshold trials proving `max_source_file_lines=658` passes while `657` fails on `src/app/services/proposal_service.py`. Full local `make check` passed with ruff, format check, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 544 source files, Workbench/OpenAPI contract smoke, and 1,225 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209 integration tests, 1,434 combined coverage tests, 94.33% total coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Run GitHub gates before merge |
...
| Largest source file | 2,968 lines | 658 script-counted lines | Improved and protected by the 658-line blocking threshold; `src/app/services/proposal_service.py` is the largest residual hotspot after reducing `src/app/contracts/reporting_batches.py` to a 75-line compatibility facade |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 658-line blocking ceiling.
