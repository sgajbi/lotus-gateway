...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current upload-artifact runtime branch keeps the blocking refactor threshold gate green, raises the governed `actions/upload-artifact` baseline to `v7`, and keeps workflow-level `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24`; previous local PR-grade evidence is `make check` with 1,112 unit/contract tests and `make ci` with 207 integration tests, 1,319 coverage tests, 94.13% total coverage, and dependency audit | Keep Docker parity blocking in PR Merge Gate |
