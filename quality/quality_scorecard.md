...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current portfolio workspace contract-boundary branch ratchets the blocking refactor threshold from 754 to 742 script-counted lines after moving portfolio workspace response, profile, rebalance, reporting, operations, and control-capability contracts into `src/app/contracts/portfolio_workspace.py`. Focused validation passed with ruff check, ruff format check, 19 focused workspace contract/response/control/OpenAPI/threshold tests, and the refactor-threshold gate at `max_source_file_lines=742`. Full local `make check` passed with ruff, format check over 731 files, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 518 source files, OpenAPI smoke, and 1,193 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209 integration tests, 1,402 combined coverage tests, 94.26% total coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Keep Docker parity blocking in PR Merge Gate |
...
| Largest source file | 2,968 lines | 742 script-counted lines | Improved and protected by the 742-line blocking threshold; `src/app/services/advisor_brief_source.py` is the largest residual hotspot after reducing `src/app/contracts/portfolio.py` from 754 to 281 script-counted lines and moving workspace contracts into `src/app/contracts/portfolio_workspace.py` |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 742-line blocking ceiling.
