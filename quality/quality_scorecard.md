...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Advise workspace client-boundary branch ratchets the blocking refactor threshold from 1,062 to 1,041 script-counted lines after reducing `advise_client.py` below the prior ceiling. Focused ruff, format, touched-client mypy, 179 upstream-client/boundary tests, and refactor-threshold gates passed. Full local `make check` passed with ruff, format check, monetary-float guard, refactor threshold gate, workflow action-runtime gate, mypy over 492 source files, Workbench/OpenAPI contract smoke, and 1,165 unit/contract tests. Full local `make ci` passed with 207 integration tests, 1,372 combined coverage tests, 94.23% total coverage, migration contract smoke, and `pip-audit` reporting no known vulnerabilities after the governed `PYSEC-2026-161` exception | Keep Docker parity blocking in PR Merge Gate |
