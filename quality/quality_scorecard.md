...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 329-line source-file ceiling. This slice moves Advise proposal narrative route calls into `src/app/clients/advise_proposal_narrative_client.py` while preserving the public `AdviseClient` facade. The evidence-selected largest residual hotspot is now `src/app/services/workspace_client_protocols.py` at 329 lines. | Continue with the next cohesive 329-line hotspot slice |
...
| Largest source file | 2,968 lines | 329 lines | Improved and protected by the 329-line blocking threshold; `src/app/services/workspace_client_protocols.py` is the evidence-selected largest residual hotspot after extracting Advise proposal narrative routes |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 329/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 329-line blocking ceiling.
