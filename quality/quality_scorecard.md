...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Advise proposal client-boundary branch ratchets the blocking refactor threshold from 712 to 709 script-counted lines after moving proposal lifecycle and memo forwarding into `advise_proposal_client.py`. Focused validation passed with ruff check, ruff format check, mypy over the touched client modules, 28 Advise client boundary/upstream tests, 4 refactor-threshold tests, and refactor-threshold trials proving `max_source_file_lines=709` passes while `708` fails on `src/app/contracts/risk_workspace.py`. Full local `make check` passed with ruff, format check over 737 files, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 523 source files, OpenAPI smoke, and 1,205 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209 integration tests, 1,414 combined coverage tests, 94.29% total coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Keep Docker parity blocking in PR Merge Gate |
...
| Largest source file | 2,968 lines | 709 script-counted lines | Improved and protected by the 709-line blocking threshold; `src/app/contracts/risk_workspace.py` is the largest residual hotspot after reducing `src/app/clients/advise_client.py` to 220 script-counted lines |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 709-line blocking ceiling.
