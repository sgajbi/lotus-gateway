...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 334-line source-file ceiling. This slice moves performance detail capability policy into `src/app/services/performance_workspace_detail_capabilities.py` and shared module capability construction into `src/app/services/performance_workspace_module_capability.py` while preserving the public performance workspace capability facade. The evidence-selected largest residual hotspot is now `src/app/services/foundation_core_snapshot.py` at 334 lines. | Continue with the next cohesive 334-line hotspot slice |
...
| Largest source file | 2,968 lines | 334 lines | Improved and protected by the 334-line blocking threshold; `src/app/services/foundation_core_snapshot.py` is the evidence-selected largest residual hotspot after extracting performance detail capabilities |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 334/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 334-line blocking ceiling.
