...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 344-line source-file ceiling. This slice moves advisor-brief review-action context loading and submission into `src/app/services/advisor_brief_review_actions.py` while preserving the public `AdvisorBriefService` API. The largest residual hotspot is now `src/app/services/advisor_brief_workflow_pack.py` at 344 lines. | Continue with the next cohesive 344-line hotspot slice |
...
| Largest source file | 2,968 lines | 344 lines | Improved and protected by the 344-line blocking threshold; `src/app/services/advisor_brief_workflow_pack.py` is the evidence-selected largest residual hotspot after extracting advisor-brief review-action orchestration |
...
| Local unit/contract tests | 996 | 1,382 | Added focused proposal lifecycle contract-boundary and lifecycle query service-boundary pins plus focused advisor-brief review-action boundary, performance calculation evidence boundary/completion tests, performance workspace context/boundary tests, response-service boundary test, portfolio projected-cashflow service-boundary test, risk concentration schema-example boundary test, DPM router-group facade and route-family export boundary tests, Workbench rebalance snapshot boundary test, risk drawdown payload-boundary test, performance contribution payload-boundary test, risk workspace response-loading boundary test, and the existing focused contract/service boundary coverage |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 344/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 344-line blocking ceiling.
