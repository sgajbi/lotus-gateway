...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 336-line source-file ceiling. This slice moves analytics audit-log emitters into `src/app/observability/analytics_ui_audit.py` while preserving the public `analytics_ui` import surface. The largest residual hotspot is now `src/app/contracts/risk_workspace_attribution.py` at 336 lines. | Continue with the next cohesive 336-line hotspot slice |
...
| Largest source file | 2,968 lines | 336 lines | Improved and protected by the 336-line blocking threshold; `src/app/contracts/risk_workspace_attribution.py` is the evidence-selected largest residual hotspot after extracting analytics UI audit logs |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 336/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 336-line blocking ceiling.
