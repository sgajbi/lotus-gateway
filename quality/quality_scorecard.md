...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current risk workspace attribution service branch moves attribution request normalization, blocked-response handling, cache orchestration, upstream fan-out, and response mapping into a focused mixin while preserving the public `RiskWorkspaceService.get_attribution` surface. Focused validation passed with 70 risk workspace service, service-boundary, service-factory, provider, wiki, and quality-threshold tests. The quality-baseline workflow now enforces the 549/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Local `make check` passed with workflow governance, mypy over 573 source files, OpenAPI smoke, and 1,260 unit/contract tests. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Monitor GitHub feature-lane and quality-baseline gates before merge |
...
| Largest source file | 2,968 lines | 549 script-counted lines | Improved and protected by the 549-line blocking threshold; `src/app/services/dpm_pm_operating_quality_service.py` is the single largest residual hotspot after splitting risk workspace attribution orchestration |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 549/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 549-line blocking ceiling.
