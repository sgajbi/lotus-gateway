...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current performance workspace boundary branch ratchets the blocking refactor threshold from 639 to 632 script-counted lines after moving detail-view orchestration into `performance_workspace_detail_views.py`. Focused validation passed with 69 targeted performance workspace/service-boundary/threshold tests. Refactor-threshold trials prove `max_source_file_lines=632` passes while `631` fails on `src/app/router_registry.py` and `src/app/services/risk_workspace_service.py`. Full local `make check` and `make ci` are green for this branch. | Monitor GitHub checks before merge |
...
| Security | 4/5 | Current performance workspace boundary branch introduces no dependency, authentication, caller-context, product-error-detail, upstream error-shape, or monetary-float conversion changes. Full local `make ci` passed `pip-audit` with no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Triage bandit and sensitive-data handling checks |
...
| Operations readiness | 4/5 | Existing CI/runbook docs and wiki validation posture now distinguish report-only quality baselines from blocking refactor-threshold and workflow action-runtime gates, including the workflow-level Node 24 JavaScript action runtime opt-in. Latest clean-main canonical proof passed `npm run live:validate`: `lotus-workbench/output/playwright/live-canonical/live-validation-summary.json` was generated at `2026-06-18T16:04:19.273Z` with 94 API checks, 29 screenshots, 25 ready panel classifications, 2 calculation checks, and no missing or non-ready panels. This branch moves internal performance workspace detail-view orchestration behind a helper module and does not change runtime routing, downstream clients, response fields, dependencies, or caller-context/security handling. | Add incident playbooks and SLO checks |
...
| Largest source file | 2,968 lines | 632 script-counted lines | Improved and protected by the 632-line blocking threshold; `src/app/router_registry.py` and `src/app/services/risk_workspace_service.py` are the largest residual hotspots after extracting performance workspace detail-view orchestration |
