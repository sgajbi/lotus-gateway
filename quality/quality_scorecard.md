...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current portfolio liquidity contract-boundary branch ratchets the blocking refactor threshold from 954 to 951 script-counted lines after moving liquidity and projected-cashflow contracts into `portfolio_liquidity.py` and preserving `app.contracts.portfolio` as the compatibility facade. Focused validation passed with ruff check, ruff format check, 14 portfolio/refactor boundary tests, and refactor-threshold trials proving `max_source_file_lines=951` passes while `950` fails on `src/app/services/foundation_service.py`. Full local `make check` passed with 1,169 unit/contract tests, and full local `make ci` passed with 207 integration tests, 1,376 combined coverage tests, 94.22% total coverage, migration contract smoke, and pip-audit reporting no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Keep Docker parity blocking in PR Merge Gate |
...
| Security | 4/5 | Current portfolio liquidity contract-boundary branch introduces no dependency, authentication, caller-context, monetary-float, or product-error-detail policy changes; current full local `make ci` `pip-audit` reported no known vulnerabilities after the governed `PYSEC-2026-161` exception | Triage bandit and sensitive-data handling checks |
...
| Largest source file | 2,968 lines | 951 script-counted lines | Improved and protected by the 951-line blocking threshold; `src/app/services/foundation_service.py` is the largest residual hotspot after reducing `src/app/contracts/portfolio.py` to 754 script-counted lines |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 951-line blocking ceiling.
