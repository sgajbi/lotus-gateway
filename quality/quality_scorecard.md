...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current DPM wave client-boundary branch splits Manage rebalance-wave core, campaign-definition, and campaign-workflow route forwarding into focused client mixins while preserving the `DpmWaveClientMixin` compatibility facade. Focused validation passed with ruff over touched files, 8 DPM client/threshold tests, refactor-threshold proof at `max_source_file_lines=623`, and full local `make check` with 1,239 unit/contract tests. The quality-baseline workflow now enforces the 623/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Full local `make ci`, GitHub checks, and post-merge wiki publication remain pending for this branch. | Run full local and GitHub gates before merge |
...
| Largest source file | 2,968 lines | 623 script-counted lines | Improved and protected by the 623-line blocking threshold; `src/app/clients/lotus_analytics_client.py` is the single largest residual hotspot after splitting DPM wave client route families |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 623/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 623-line blocking ceiling.
