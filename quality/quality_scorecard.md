...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 346-line source-file ceiling. This slice moves portfolio projected-cashflow orchestration into `src/app/services/portfolio_projected_cashflow_service.py` and shared holdings upstream access typing into `src/app/services/portfolio_holdings_upstream.py` while preserving the public `PortfolioService` surface. The largest residual hotspot is now `src/app/services/performance_calculation_evidence.py` at 346 lines. | Continue with the next cohesive 346-line hotspot slice |
...
| Largest source file | 2,968 lines | 346 lines | Improved and protected by the 346-line blocking threshold; `src/app/services/performance_calculation_evidence.py` is the evidence-selected largest residual hotspot after extracting portfolio projected-cashflow orchestration |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 346/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 346-line blocking ceiling.
