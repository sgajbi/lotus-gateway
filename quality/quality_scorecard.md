...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current DPM PM operating-quality summary branch moves Manage score-run evidence loading, Lotus AI workflow-pack execution, missing-score-run validation, and summary response assembly into a focused mixin while preserving the public `request_pm_operating_quality_summary` surface. Focused validation passed with 105 DPM command-center service, AI context, supportability, router, service-boundary, wiki, and quality-threshold tests. The quality-baseline workflow now enforces the 536/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Local `make check` passed with workflow governance, mypy over 574 source files, OpenAPI smoke, and 1,261 unit/contract tests. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Monitor GitHub feature-lane and quality-baseline gates before merge |
...
| Largest source file | 2,968 lines | 536 script-counted lines | Improved and protected by the 536-line blocking threshold; `src/app/clients/advise_proposal_client.py` is the single largest residual hotspot after splitting DPM PM operating-quality summary workflow orchestration |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 536/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 536-line blocking ceiling.
