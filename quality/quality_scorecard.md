...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 335-line source-file ceiling. This slice moves HTTP response and failure payload helpers into `src/app/clients/http_response_payloads.py` while preserving the public retry helper behavior. The evidence-selected largest residual hotspot is now `src/app/contracts/risk_workspace_rolling.py` at 335 lines, with `performance_workspace_capabilities.py` tied at the same ceiling. | Continue with the next cohesive 335-line hotspot slice |
...
| Largest source file | 2,968 lines | 335 lines | Improved and protected by the 335-line blocking threshold; `src/app/contracts/risk_workspace_rolling.py` is the evidence-selected largest residual hotspot after extracting HTTP response payload helpers |
