...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Lotus Core lookup-client boundary branch splits lookup forwarding into a focused mixin while preserving the `LotusCoreQueryClient` surface. Local `make check` passed with workflow governance, mypy over 563 source files, OpenAPI smoke, and 1,246 unit/contract tests. The quality-baseline workflow now enforces the 606/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Run GitHub gates before merge |
...
| Largest source file | 2,968 lines | 606 script-counted lines | Improved and protected by the 606-line blocking threshold; `src/app/services/dpm_client_protocols.py` is the single largest residual hotspot after splitting Lotus Core lookup forwarding |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 606/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 606-line blocking ceiling.
