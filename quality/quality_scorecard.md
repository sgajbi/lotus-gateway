...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current workbench enrichment boundary branch ratchets the blocking refactor threshold from 685 to 680 script-counted lines after moving Workbench overview performance/rebalance enrichment into `workbench_overview_enrichment.py`. Focused validation passed with ruff check, ruff format check, mypy over the touched Workbench service/enrichment modules, 114 focused Workbench/refactor tests, and refactor-threshold trials proving `max_source_file_lines=680` passes while `679` fails on `src/app/services/portfolio_service.py`. Full local `make check` passed with ruff, format check over 748 files, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 530 source files, OpenAPI smoke, and 1,219 unit/contract tests; this branch still requires fresh `make ci`, Docker rebuild, and live canonical validation before PR/merge. | Run fresh `make ci`, Docker rebuild, and live canonical validation before PR/merge |
...
| Largest source file | 2,968 lines | 680 script-counted lines | Improved and protected by the 680-line blocking threshold; `src/app/services/portfolio_service.py` is the largest residual hotspot after reducing `src/app/services/workbench_service.py` to 562 script-counted lines |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 680-line blocking ceiling.
