...
| Security | 4/5 | Current foundation core-snapshot mapper branch introduces no dependency, authentication, caller-context, monetary-float, or product-error-detail policy changes; prior full local `make ci` `pip-audit` reported no known vulnerabilities after the governed `PYSEC-2026-161` exception, and full local gates remain required before PR/merge | Triage bandit and sensitive-data handling checks |
