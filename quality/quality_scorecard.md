...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 335-line source-file ceiling. This slice moves rolling metric and window contracts into `src/app/contracts/risk_workspace_rolling_metrics.py` while preserving the public rolling contract facade. The evidence-selected largest residual hotspot is now `src/app/services/performance_workspace_capabilities.py` at 335 lines. | Continue with the next cohesive 335-line hotspot slice |
...
| Largest source file | 2,968 lines | 335 lines | Improved and protected by the 335-line blocking threshold; `src/app/services/performance_workspace_capabilities.py` is the evidence-selected largest residual hotspot after extracting rolling metric contracts |
