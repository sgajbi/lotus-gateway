...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Bank-Buyable source-file threshold ratchet branch keeps the blocking refactor threshold gate green at the 2,000-line source-file ceiling; `make check` passed with ruff, format check, monetary-float guard, the refactor threshold gate, mypy over 472 source files, Workbench/OpenAPI contract smoke, and 1,090 unit/contract tests; `make ci` passed with 207 integration tests, 1,297 coverage tests, 94.11% total coverage, and dependency audit | Keep Docker parity blocking in PR Merge Gate |
...
| Security | 4/5 | Current source-file threshold ratchet branch `pip-audit` found no known vulnerabilities after the governed `PYSEC-2026-161` exception; monetary-float guard passed with the existing governed allowlist | Triage bandit and sensitive-data handling checks |
...
versus the current source-file threshold ratchet branch after PRs #349, #350, #351, #352,
...
| --- | ---: | ---: | --- |
| Counted files under `src`, `tests`, `docs`, `wiki`, `.github`, `scripts` | 1,265 | 1,391 | Added focused modules, tests, quality evidence, and wiki source |
