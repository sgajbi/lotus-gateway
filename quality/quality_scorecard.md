...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current advisor-brief contract-boundary branch ratchets the blocking refactor threshold from 646 to 639 script-counted lines after moving Advisor Brief presentation/source and source-supportability contracts into dedicated modules. Focused validation passed with 39 contract-boundary, threshold, Advisor Brief service/supportability, and Workbench contract tests, plus mypy over 547 source files. Refactor-threshold trials prove `max_source_file_lines=639` passes while `638` fails on `src/app/services/performance_workspace_service.py`. Full local `make check` passed with ruff, format check over 767 files, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 547 source files, Workbench/OpenAPI contract smoke, and 1,227 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209 integration tests, 1,436 combined coverage tests, 94.33% total coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Run GitHub gates before merge |
| Coverage | 4/5 | 94.33% total coverage across 1,436 combined coverage tests, above the 84% floor in current local `make ci` evidence | Add targeted middleware/security/error tests |
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 639-line source-file ceiling; recent slices extracted Workbench common/overview/sandbox contract families, performance calculation evidence helpers, risk workspace request-context helpers, portfolio workspace contract family, portfolio workflow orchestration, advisor-brief workflow-pack contracts, Advisor Brief item/source contracts, Advisor Brief source-supportability contracts, Advisor Brief client protocols, proposal lifecycle contracts, proposal lifecycle transition orchestration, DPM PM operating-quality contracts, performance workspace context-service policy, portfolio readiness response construction, performance evidence artifact handling, portfolio upstream payload handling, portfolio source-loading/mapping helpers, DPM route families, proposal memo contracts, portfolio liquidity response assembly, Foundation core snapshot parsing, performance horizon-comparison contracts, Advise-owned router groups, advisor-brief AI narrative mapping, risk workspace response OpenAPI examples, analytics workspace-summary payload construction, portfolio catalog response loading, Workbench overview enrichment orchestration, portfolio insights response assembly, performance horizon row assembly, reporting query contract families, performance workspace response contracts, and reporting batch contract families into dedicated modules. `src/app/contracts/advisor_brief.py` is reduced from 646 to 398 script-counted lines, making `src/app/services/performance_workspace_service.py` the largest residual hotspot at 639 script-counted lines. | Continue extraction slices and tighten the source-file ceiling downward |
...
| Security | 4/5 | Current advisor-brief contract-boundary branch introduces no dependency, authentication, caller-context, product-error-detail, upstream error-shape, or monetary-float conversion changes. Finding count remains 159 and the monetary-float guard passes. Full local `make ci` security audit found no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Triage bandit and sensitive-data handling checks |
...
| Operations readiness | 4/5 | Existing CI/runbook docs and wiki validation posture now distinguish report-only quality baselines from blocking refactor-threshold and workflow action-runtime gates, including the workflow-level Node 24 JavaScript action runtime opt-in. Latest clean-main canonical proof passed `npm run live:validate`: `lotus-workbench/output/playwright/live-canonical/live-validation-summary.json` was generated at `2026-06-18T16:04:19.273Z` with 94 API checks, 29 screenshots, 25 ready panel classifications, 2 calculation checks, and no missing or non-ready panels. This branch is contract-only and does not change runtime routing, service orchestration, downstream calls, or response fields. | Add incident playbooks and SLO checks |
...
| Largest source file | 2,968 lines | 639 script-counted lines | Improved and protected by the 639-line blocking threshold; `src/app/services/performance_workspace_service.py` is the largest residual hotspot after reducing `src/app/contracts/advisor_brief.py` to 398 script-counted lines |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 639-line blocking ceiling.
