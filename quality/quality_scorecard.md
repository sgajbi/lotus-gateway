...
| Security | 4/5 | Current risk rolling contract branch `pip-audit` passed with the governed FastAPI/Starlette exception; monetary-float guard passed after refreshing the moved rolling metric-value allowlist path | Triage bandit and sensitive-data handling checks |
| Documentation | 4/5 | Baseline, scorecard, health report, and wiki validation evidence are refreshed to current branch metrics after the current focused risk rolling contract slice | Keep wiki synced and add diagrams over time |
...
versus the current risk rolling contract branch after PRs #349, #350, #351, #352, #353, #354,
#355, #356, #357, #358, #359, #360, #361, #362, #363, #364, #365, #366, #367, and #368.

| Measure | Prior scorecard | Current branch | Result |
| --- | ---: | ---: | --- |
| Counted files under `src`, `tests`, `docs`, `wiki`, `.github`, `scripts` | 1,265 | 1,361 | Added focused modules, tests, and quality evidence |
| Tracked `src/app` Python files | 447 | 467 | Added focused portfolio/performance response helpers and workspace/readiness/transaction summary/workflow mapper and contract modules plus risk drawdown, reporting batch, reporting query, risk concentration, and risk rolling contracts |
| Tracked Python test files | 162 | 179 | Added focused request-context, response-assembly, workspace parser, source-readiness parser, transaction summary mapper, workflow mapper, workflow contract, transaction contract, performance snapshot contract, income/activity contract, holdings contract, risk drawdown contract, reporting batch contract, reporting query contract, risk concentration contract, and risk rolling contract tests |
| Longest function | 49 lines | 49 lines | Preserved |
| Top function hotspot count at 49 lines | 2 | 2 | Preserved |
| Largest source file | 2,968 lines | 1,970 lines | Improved; `src/app/services/portfolio_service.py` remains the largest residual hotspot after reducing `portfolio.py`, `risk_workspace.py` to 969 lines, and `reporting.py` |
...
| Local unit/contract tests | 996 | 1,052 | Added focused response/request boundary, workspace parser, source-readiness parser, transaction summary mapper, workflow mapper, workflow contract, transaction contract, performance snapshot contract, income/activity contract, holdings contract, risk drawdown contract, reporting batch contract, reporting query contract, risk concentration contract, and risk rolling contract tests |
| Local coverage tests | 1,203 | 1,259 | Added focused coverage while preserving total coverage |
