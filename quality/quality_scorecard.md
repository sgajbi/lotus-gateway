...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 328-line source-file ceiling. This slice moves Foundation upstream protocol families into `src/app/services/foundation_client_protocols.py` while preserving the public `workspace_client_protocols` compatibility facade. The evidence-selected largest residual hotspot is now `src/app/services/risk_workspace_concentration.py` at 328 lines. | Continue with the next cohesive 328-line hotspot slice |
...
| Largest source file | 2,968 lines | 328 lines | Improved and protected by the 328-line blocking threshold; `src/app/services/risk_workspace_concentration.py` is the evidence-selected largest residual hotspot after extracting Foundation protocol families |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 328/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 328-line blocking ceiling.
