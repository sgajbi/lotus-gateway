...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current portfolio workspace source-loading branch keeps the blocking refactor threshold and workflow action-runtime gates green; focused evidence is 44 portfolio workspace/service unit tests plus ruff, format, touched-module mypy, and refactor threshold checks. Full local PR-grade evidence is recorded before merge | Keep Docker parity blocking in PR Merge Gate |
...
| Security | 4/5 | Current portfolio workspace source-loading branch introduces no dependency, network, authentication, or monetary-float policy changes; prior full local audit found no known vulnerabilities after the governed `PYSEC-2026-161` exception, and full `make ci` remains a pre-merge gate | Triage bandit and sensitive-data handling checks |
