...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current performance contribution contract branch `make check` passed with ruff, format check, monetary-float guard, mypy over 469 source files, Workbench/OpenAPI contract smoke, and 1,056 unit/contract tests; `make ci` passed with 207 integration tests, 1,263 coverage tests, 94.04% total coverage, and dependency audit | Keep Docker parity blocking in PR Merge Gate |
| Coverage | 4/5 | 94.04% total coverage, above the 84% floor | Add targeted middleware/security/error tests |
| Modularity | 4/5 | Current branch state preserves the 49-line longest-function baseline; recent slices extracted transaction request context, transaction page-context defaults, transaction client-kwargs mapping, performance workspace response assembly, portfolio workspace response assembly, portfolio workspace performance parsing, portfolio workspace rebalance parsing, portfolio source-readiness parsing, portfolio transaction summary mapping, portfolio workflow mapping, portfolio workflow contracts, portfolio transaction contracts, portfolio performance snapshot contracts, portfolio income/activity contracts, portfolio holdings/book contracts, risk drawdown contracts, reporting batch contracts, reporting query contracts, risk concentration contracts, risk rolling contracts, risk attribution contracts, and performance contribution contracts; `portfolio_service.py` is now 1,970 measured lines, `performance_workspace.py` is now 1,499 measured lines, `portfolio.py` is now 911 measured lines, `risk_workspace.py` is now 678 measured lines, and `reporting.py` is now 532 measured lines | Continue extraction slices and reduce remaining largest-file pressure |
...
| Security | 4/5 | Current performance contribution contract branch `pip-audit` passed with the governed FastAPI/Starlette exception; monetary-float guard passed after refreshing moved performance contribution-field allowlist paths | Triage bandit and sensitive-data handling checks |
| Documentation | 4/5 | Baseline, scorecard, health report, and wiki validation evidence are refreshed to current branch metrics after the current focused performance contribution contract slice | Keep wiki synced and add diagrams over time |
...
versus the current performance contribution contract branch after PRs #349, #350, #351, #352,
#353, #354, #355, #356, #357, #358, #359, #360, #361, #362, #363, #364, #365, #366, #367,
#368, #369, and #370.

| Measure | Prior scorecard | Current branch | Result |
| --- | ---: | ---: | --- |
| Counted files under `src`, `tests`, `docs`, `wiki`, `.github`, `scripts` | 1,265 | 1,371 | Added focused modules, tests, and quality evidence |
| Tracked `src/app` Python files | 447 | 469 | Added focused portfolio/performance response helpers and workspace/readiness/transaction summary/workflow mapper and contract modules plus risk drawdown, reporting batch, reporting query, risk concentration, risk rolling, risk attribution, and performance contribution contracts |
| Tracked Python test files | 162 | 181 | Added focused request-context, response-assembly, workspace parser, source-readiness parser, transaction summary mapper, workflow mapper, workflow contract, transaction contract, performance snapshot contract, income/activity contract, holdings contract, risk drawdown contract, reporting batch contract, reporting query contract, risk concentration contract, risk rolling contract, risk attribution contract, and performance contribution contract tests |
| Longest function | 49 lines | 49 lines | Preserved |
| Top function hotspot count at 49 lines | 2 | 2 | Preserved |
| Largest source file | 2,968 lines | 1,970 lines | Improved; `src/app/services/portfolio_service.py` remains the largest residual hotspot after reducing `performance_workspace.py` to 1,499 lines, `portfolio.py` to 911 lines, `risk_workspace.py` to 678 lines, and `reporting.py` to 532 lines |
| `performance_workspace.py` | 1,539 lines | 1,499 lines | Improved through performance contribution contract extraction |
| OpenAPI operations with missing summary/description/tags/errors | 0 | 0 | Preserved |
| Local unit/contract tests | 996 | 1,056 | Added focused response/request boundary, workspace parser, source-readiness parser, transaction summary mapper, workflow mapper, workflow contract, transaction contract, performance snapshot contract, income/activity contract, holdings contract, risk drawdown contract, reporting batch contract, reporting query contract, risk concentration contract, risk rolling contract, risk attribution contract, and performance contribution contract tests |
| Local coverage tests | 1,203 | 1,263 | Added focused coverage while preserving total coverage |
| Total coverage | 93.69% | 94.04% | Improved |
