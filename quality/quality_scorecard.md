...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current performance evidence-boundary branch ratchets the blocking refactor threshold from 771 to 769 script-counted lines after extracting calculation evidence artifact retrieval, lineage polling, execution refresh, payload normalization, stage/snapshot mapping, and artifact URL construction into `performance_calculation_evidence.py` behind the public `performance_workspace_evidence.py` facade. Focused validation passed with ruff check, ruff format check, 59 focused performance evidence/workspace/threshold tests, and the refactor-threshold gate at `max_source_file_lines=769`. Full local `make check` passed with ruff, format check over 729 files, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 517 source files, OpenAPI smoke, and 1,190 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209 integration tests, 1,399 combined coverage tests, 94.24% total coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Keep Docker parity blocking in PR Merge Gate |
...
| Security | 4/5 | Current performance evidence-boundary branch introduces no dependency, authentication, caller-context, or product-error-detail policy changes; monetary-float allowlist remains at 159 governed findings; full local `make ci` security audit reported no known vulnerabilities after the governed `PYSEC-2026-161` exception | Triage bandit and sensitive-data handling checks |
...
| Largest source file | 2,968 lines | 769 script-counted lines | Improved and protected by the 769-line blocking threshold; `src/app/services/risk_workspace_service.py` is the largest residual hotspot after reducing `src/app/services/performance_workspace_evidence.py` from 771 to 461 script-counted lines |
...
| Local coverage tests | 1,203 | 1,399 | Added focused coverage while preserving total coverage |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 769-line blocking ceiling.
