...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | The proposal decision-vocabulary slice adds a blocking contract gate to `make lint`, requires a current producer source rather than allowing packaged self-comparison, reconciles the Advise artifact in Remote Feature, PR Merge, and Main Releasability, and adds a daily/operator-dispatched drift lane. Pairing drift reports the changed status/gate and producer blob revision before release; the explicit offline snapshot command is not CI evidence. Existing 316/49 refactor, workflow-governance, artifact-integrity, and producer-status controls remain enforced; `scripts/check_agent_quality_evidence.py` continues to guard their durable scorecard and context evidence. | Monitor the protected and scheduled vocabulary gates after push |
| Coverage | 4/5 | 94.79% total coverage across 2,261 combined unit, contract, and integration tests on the proposal vocabulary final head, above the 84% floor and the 94.77% no-regression ratchet | Add targeted middleware/security/error tests |
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 316-line source-file ceiling. This slice keeps the detail-currency response contract explicit while preserving the focused workspace seams. The evidence-selected largest residual hotspot is now `src/app/contracts/performance_workspace_details_contract.py` at 316 lines. | Continue with the next cohesive 316-line hotspot slice |
| Duplication | 3/5 | The pinned jscpd quality tool now measures 88 production clone findings, 1,767 duplicated lines, and 2.03% duplicated lines, with Python-version-independent AST-scope/canonical-source-side/source-pair/literal-preserving occurrence fingerprints, a no-new-regression ratchet, and a repeated-run candidate-set gate. The protected baseline is generated and enforced on Ubuntu/Node 20; cross-operating-system equivalence remains intentionally unclaimed. Existing clones remain explicit debt; this slice does not pretend they are cleaned up. | Remove or consolidate reviewed production clones while preserving the ratchet |
...
| Largest source file | 2,968 lines | 316 lines | Improved and protected by the 316-line blocking threshold; `src/app/contracts/performance_workspace_details_contract.py` is the evidence-selected largest residual hotspot after the detail-currency contract correction |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 316/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 316-line blocking ceiling.
