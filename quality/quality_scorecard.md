...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Foundation optional-workspace boundary branch splits optional performance, rebalance, reporting, evidence-summary, and workflow-cue parsing into a focused module while preserving the public `FoundationService` API surface. Focused validation passed with 30 foundation/quality tests. Local `make check` passed with workflow governance, mypy over 566 source files, OpenAPI smoke, and 1,249 unit/contract tests. The quality-baseline workflow now enforces the 589/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Run GitHub gates before merge |
...
| Largest source file | 2,968 lines | 589 script-counted lines | Improved and protected by the 589-line blocking threshold; `src/app/services/portfolio_service.py` is the single largest residual hotspot after splitting Foundation optional-workspace parsing |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 589/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 589-line blocking ceiling.
