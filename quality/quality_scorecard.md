...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current DPM wave protocol-family boundary branch splits `DpmWaveClient` into a focused protocol module and updates wave services to import that module directly. Local `make check` passed with workflow governance, mypy over 564 source files, OpenAPI smoke, and 1,248 unit/contract tests. The quality-baseline workflow now enforces the 595/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Run GitHub gates before merge |
...
| Largest source file | 2,968 lines | 595 script-counted lines | Improved and protected by the 595-line blocking threshold; `src/app/contracts/dpm_command_center.py` is the single largest residual hotspot after splitting DPM wave client protocols |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 595/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 595-line blocking ceiling.
