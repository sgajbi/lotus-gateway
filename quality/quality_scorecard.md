...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 326-line source-file ceiling. This slice moves concentration block extraction and supportability narration into `src/app/services/risk_workspace_concentration_supportability.py` while preserving the public concentration response mapper behavior. The evidence-selected largest residual hotspot is now `src/app/contracts/platform_capabilities.py` at 326 lines. | Continue with the next cohesive 326-line hotspot slice |
...
| Largest source file | 2,968 lines | 326 lines | Improved and protected by the 326-line blocking threshold; `src/app/contracts/platform_capabilities.py` is the evidence-selected largest residual hotspot after extracting concentration supportability mapping |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 326/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 326-line blocking ceiling.
