...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current analytics risk-client boundary branch splits risk analytics forwarding into a focused mixin while preserving the `LotusAnalyticsClient` surface. Local `make check` passed with workflow governance, mypy over 561 source files, OpenAPI smoke, and 1,243 unit/contract tests. The quality-baseline workflow now enforces the 618/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Run GitHub gates before merge |
...
| Largest source file | 2,968 lines | 618 script-counted lines | Improved and protected by the 618-line blocking threshold; `src/app/services/foundation_service.py` is the single largest residual hotspot after splitting analytics risk client route families |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 618/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 618-line blocking ceiling.
