...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current reporting job contract branch splits report-job request, error, handle, and status DTOs into a focused module while preserving the public `app.contracts.reporting` import surface. Focused validation passed with 59 reporting job contract, submission, query, batch compatibility, contract-boundary, reporting router, wiki, and quality-threshold tests. The quality-baseline workflow now enforces the 559/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Local `make check` passed with workflow governance, mypy over 571 source files, OpenAPI smoke, and 1,258 unit/contract tests. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Monitor GitHub feature-lane and quality-baseline gates before merge |
...
| Largest source file | 2,968 lines | 559 script-counted lines | Improved and protected by the 559-line blocking threshold; `src/app/clients/lotus_analytics_client.py` is the single largest residual hotspot after splitting reporting job contracts |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 559/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 559-line blocking ceiling.
