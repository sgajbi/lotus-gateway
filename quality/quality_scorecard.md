...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current analytics performance client branch splits TWR, MWR, composite, contribution, attribution, lineage, and workspace-summary route methods into a focused mixin while preserving the public `LotusAnalyticsClient` surface. Focused validation passed with 210 analytics client, factory, upstream-client, composite-performance, platform-capabilities, wiki, and quality-threshold tests. The quality-baseline workflow now enforces the 556/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Local `make check` passed with workflow governance, mypy over 572 source files, OpenAPI smoke, and 1,259 unit/contract tests. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Monitor GitHub feature-lane and quality-baseline gates before merge |
...
| Largest source file | 2,968 lines | 556 script-counted lines | Improved and protected by the 556-line blocking threshold; `src/app/services/risk_workspace_service.py` is the single largest residual hotspot after splitting analytics performance client routes |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 556/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 556-line blocking ceiling.
