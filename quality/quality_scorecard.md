...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 315-line source-file ceiling. #664 separates static examples, descriptor state, and transaction request mapping without API change. The evidence-selected largest residual hotspot is now `src/app/contracts/dpm_pm_operating_quality.py` at 315 lines. | Continue with the next cohesive 315-line hotspot slice |
| Duplication | 3/5 | The pinned jscpd quality tool now measures 70 production clone findings, 1,388 duplicated lines, and 1.47% duplicated lines, with Python-version-independent AST-scope/canonical-source-side/source-pair/literal-preserving occurrence fingerprints, a no-new-regression ratchet, and a repeated-run candidate-set gate. The protected baseline is generated and enforced on Ubuntu/Node 20; cross-operating-system equivalence remains intentionally unclaimed. Existing clones remain explicit debt; this slice removes the consolidated advisory-policy read-route clone and banks that improvement. | Remove or consolidate reviewed production clones while preserving the ratchet |
...
| Largest source file | 2,968 lines | 315 lines | Improved and protected by the 315-line blocking threshold; `src/app/contracts/dpm_pm_operating_quality.py` is the evidence-selected largest residual hotspot after #664 |
