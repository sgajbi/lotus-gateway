...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 330-line source-file ceiling. This slice moves domain-product graph node, edge, data, and response contracts into `src/app/contracts/domain_product_graph.py` while preserving the public `domain_products` facade. The evidence-selected largest residual hotspot is now `src/app/clients/dpm_client.py` at 330 lines. | Continue with the next cohesive 330-line hotspot slice |
...
| Largest source file | 2,968 lines | 330 lines | Improved and protected by the 330-line blocking threshold; `src/app/clients/dpm_client.py` is the evidence-selected largest residual hotspot after extracting domain-product graph contracts |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 330/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 330-line blocking ceiling.
