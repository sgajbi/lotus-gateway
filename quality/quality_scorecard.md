...
| Security | 4/5 | Latest merged performance attribution slice `pip-audit` passed with the governed FastAPI/Starlette exception; current performance evidence contract branch monetary-float guard passed without allowlist churn | Triage bandit and sensitive-data handling checks |
...
versus the current performance evidence contract branch after PRs #349, #350, #351, #352,
...
| Local unit/contract tests | 996 | 1,059 | Added focused response/request boundary, workspace parser, source-readiness parser, transaction summary mapper, workflow mapper, workflow contract, transaction contract, performance snapshot contract, income/activity contract, holdings contract, risk drawdown contract, reporting batch contract, reporting query contract, risk concentration contract, risk rolling contract, risk attribution contract, performance contribution contract, performance attribution contract, and performance evidence compatibility tests |
| Local coverage tests | 1,203 | 1,266 | Added focused coverage while preserving total coverage |
