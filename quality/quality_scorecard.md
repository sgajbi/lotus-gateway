...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current portfolio holdings payload mapper branch keeps the blocking refactor threshold gate green at the 2,000-line source-file ceiling; focused validation passed with ruff format, ruff check, the refactor threshold gate, and 45 focused portfolio holdings/service unit tests; full `make check` and `make ci` evidence remains pending | Keep Docker parity blocking in PR Merge Gate |
...
| Security | 4/5 | Current portfolio holdings payload mapper branch has no security behavior change; dependency audit remains pending for the full `make ci` gate | Triage bandit and sensitive-data handling checks |
...
| Local unit/contract tests | 996 | Pending full gate | Focused portfolio holdings/service unit slice passed 45 tests; full unit/contract count pending `make check` |
| Local coverage tests | 1,203 | Pending full gate | Full combined coverage count pending `make ci` |
