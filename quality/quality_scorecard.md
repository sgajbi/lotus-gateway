...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current DPM wave AI handoff boundary branch ratchets the blocking refactor threshold from 1,093 to 1,062 script-counted lines after reducing `dpm_wave_service.py` below the prior ceiling. Focused ruff, format, touched-module mypy, 38 DPM wave service/boundary/contract/router tests, and threshold trial gates passed; full local `make check` and `make ci` evidence remains pending for this branch | Keep Docker parity blocking in PR Merge Gate |
...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 1,062-line source-file ceiling; recent slices extracted performance workspace request-context policy, portfolio workspace component parser policy, portfolio readiness response construction, performance evidence artifact handling, portfolio upstream payload handling, removed stale portfolio service wrappers, portfolio source-loading/mapping helpers, DPM PM operating-quality/construction/proof-pack upstream client route families, portfolio cached upstream access, DPM PM operating-quality service orchestration, performance trend service orchestration, Advise bank-demo proof upstream routes, and now DPM wave AI handoff orchestration into dedicated modules. `advise_client.py` is the largest residual hotspot at 1,062 script-counted lines after reducing `dpm_wave_service.py` from 1,093 to 692 script-counted lines in this slice | Continue extraction slices and tighten the source-file ceiling downward |
...
| Security | 4/5 | Current DPM wave AI handoff boundary branch introduces no dependency, authentication, caller-context, monetary-float, or product-error-detail policy changes; full local `make ci` security audit evidence remains pending for this branch | Triage bandit and sensitive-data handling checks |
...
| Tracked `src/app` Python files | 447 | 491 | Added focused portfolio/performance response helpers and workspace/readiness/transaction summary/workflow mapper and contract modules plus risk drawdown, reporting batch, reporting query, risk concentration, risk rolling, risk attribution, performance contribution, performance attribution, performance evidence, portfolio workspace payload, portfolio holdings payload, portfolio catalog payload, position-book mapper, portfolio book response, portfolio workspace source-loading, portfolio readiness/insight source-loading, portfolio book source-loading, advisor-brief source mapper modules, DPM PM operating-quality/construction/proof-pack client mixins, portfolio upstream-access mixin, DPM PM operating-quality service mixin, performance trend service mixin, Advise bank-demo proof client mixin, and DPM wave AI handoff mixin |
...
| Largest source file | 2,968 lines | 1,062 script-counted lines | Improved and protected by the 1,062-line blocking threshold; `src/app/clients/advise_client.py` is the largest residual hotspot after extracting DPM wave AI handoff orchestration and reducing `dpm_wave_service.py` to 692 script-counted lines |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 1,062-line blocking ceiling.
