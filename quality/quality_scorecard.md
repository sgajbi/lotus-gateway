...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Bank-Buyable quality-baseline artifact branch keeps the blocking refactor threshold gate green and enforces quality-baseline evidence artifact capture; `make check` passed with ruff, format check, monetary-float guard, the refactor threshold gate, mypy over 472 source files, Workbench/OpenAPI contract smoke, and 1,089 unit/contract tests; `make ci` passed with 207 integration tests, 1,296 coverage tests, 94.11% total coverage, and dependency audit | Keep Docker parity blocking in PR Merge Gate |
| Coverage | 4/5 | 94.11% total coverage, above the 84% floor | Add targeted middleware/security/error tests |
...
| Security | 4/5 | Current Bank-Buyable quality-baseline artifact branch `pip-audit` found no known vulnerabilities after the governed `PYSEC-2026-161` exception; monetary-float guard passed with the existing governed allowlist | Triage bandit and sensitive-data handling checks |
...
| Largest source file | 2,968 lines | 1,929 physical lines | Improved and now protected by the 2,000-line blocking threshold; `src/app/services/portfolio_service.py` remains the largest residual hotspot after extracting workspace payload mapping and transaction-summary context loading; `performance_workspace.py`, `portfolio.py`, `risk_workspace.py`, and `reporting.py` remain smaller residual contract hotspots |
