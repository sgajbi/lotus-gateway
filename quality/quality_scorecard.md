...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current portfolio transaction-summary context branch keeps the blocking refactor threshold gate green; `make check` passed with ruff, format check, monetary-float guard, the refactor threshold gate, mypy over 471 source files, Workbench/OpenAPI contract smoke, and 1,070 unit/contract tests; `make ci` passed with 207 integration tests, 1,277 coverage tests, 94.08% total coverage, and dependency audit | Keep Docker parity blocking in PR Merge Gate |
| Coverage | 4/5 | 94.08% total coverage, above the 84% floor | Add targeted middleware/security/error tests |
...
| Security | 4/5 | Current portfolio transaction-summary context branch `pip-audit` passed with the governed FastAPI/Starlette exception; monetary-float guard passed without allowlist churn | Triage bandit and sensitive-data handling checks |
| Documentation | 4/5 | Baseline, scorecard, health report, and wiki validation evidence are refreshed to current branch metrics after the current portfolio transaction-summary context slice | Keep wiki synced and add diagrams over time |
...
versus the current portfolio transaction-summary context branch after PRs #349, #350, #351, #352,
...
| Largest source file | 2,968 lines | 1,888 physical lines | Improved and now protected by the 2,100-line blocking threshold; `src/app/services/portfolio_service.py` remains the largest residual hotspot after extracting transaction-summary context loading and after reducing `performance_workspace.py` to 903 measured lines, `portfolio.py` to 911 measured lines, `risk_workspace.py` to 678 measured lines, and `reporting.py` to 532 measured lines |
