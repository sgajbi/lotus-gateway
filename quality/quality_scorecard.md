...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 316-line source-file ceiling. This slice moves Foundation workspace source loading into `src/app/services/foundation_workspace_sources.py`, keeping core/optional source fan-out and performance reference-date fallback out of the main Foundation response service. The evidence-selected largest residual hotspot is now `src/app/services/platform_capabilities_workspace_descriptors.py` at 316 lines. | Continue with the next cohesive 316-line hotspot slice |
...
| Largest source file | 2,968 lines | 316 lines | Improved and protected by the 316-line blocking threshold; `src/app/services/platform_capabilities_workspace_descriptors.py` is the evidence-selected largest residual hotspot after extracting Foundation workspace source loading |
