...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current DPM construction/proof-pack client-boundary branch keeps the blocking refactor threshold green at 1,237 script-counted lines without lowering the global ceiling because `portfolio_service.py` remains the top hotspot. Focused ruff, mypy, upstream-client route tests, and DPM client boundary tests passed. Local `make check` passed with ruff, format check, monetary-float guard, refactor threshold gate, workflow action-runtime gate, mypy over 486 source files, Workbench/OpenAPI contract smoke, and 1,161 unit/contract tests. Local `make ci` passed with 207 integration tests, 1,368 combined coverage tests, 94.26% total coverage, migration contract smoke, and no known vulnerabilities after the governed `PYSEC-2026-161` exception | Keep Docker parity blocking in PR Merge Gate |
...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 1,237-line source-file ceiling; recent slices extracted performance workspace request-context policy, portfolio workspace component parser policy, portfolio readiness response construction, performance evidence artifact handling, portfolio upstream payload handling, removed stale portfolio service wrappers, portfolio source-loading/mapping helpers, and now the DPM PM operating-quality, construction, and proof-pack upstream client route families into dedicated mixin modules. `portfolio_service.py` is the largest residual hotspot at 1,237 script-counted lines after reducing `dpm_client.py` from 1,121 to 1,041 physical lines in this slice | Continue extraction slices and tighten the source-file ceiling downward |
...
| Security | 4/5 | Current portfolio workspace component parser extraction branch introduces no dependency, network, authentication, caller-context, monetary-float, or error-detail policy changes; current local `make ci` found no known vulnerabilities after the governed `PYSEC-2026-161` exception | Triage bandit and sensitive-data handling checks |
...
| Tracked `src/app` Python files | 447 | 486 | Added focused portfolio/performance response helpers and workspace/readiness/transaction summary/workflow mapper and contract modules plus risk drawdown, reporting batch, reporting query, risk concentration, risk rolling, risk attribution, performance contribution, performance attribution, performance evidence, portfolio workspace payload, portfolio holdings payload, portfolio catalog payload, position-book mapper, portfolio book response, portfolio workspace source-loading, portfolio readiness/insight source-loading, portfolio book source-loading, advisor-brief source mapper modules, and the DPM PM operating-quality, construction, and proof-pack client mixins |
...
| Largest source file | 2,968 lines | 1,237 script-counted lines | Improved and protected by the 1,237-line blocking threshold; `src/app/services/portfolio_service.py` is the largest residual hotspot after extracting the DPM PM operating-quality, construction, and proof-pack client route families and reducing `dpm_client.py` below the current ceiling |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 1,237-line blocking ceiling.
