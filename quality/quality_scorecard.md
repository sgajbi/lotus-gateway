...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 344-line source-file ceiling. This slice moves advisor-brief task-flow parsing into `src/app/services/advisor_brief_task_flow.py` while preserving the public workflow-pack runtime loader. The largest residual hotspot is now `src/app/services/performance_workspace_evidence_response.py` at 344 lines. | Continue with the next cohesive 344-line hotspot slice |
...
| Largest source file | 2,968 lines | 344 lines | Improved and protected by the 344-line blocking threshold; `src/app/services/performance_workspace_evidence_response.py` is the evidence-selected largest residual hotspot after extracting advisor-brief task-flow parsing |
