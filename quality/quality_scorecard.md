...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 345-line source-file ceiling. This slice moves performance evidence completion polling into `src/app/services/performance_calculation_evidence_completion.py` while preserving the public performance evidence facade. The largest residual hotspot is now `src/app/services/advisor_brief_service.py` at 345 lines. | Continue with the next cohesive 345-line hotspot slice |
...
| Largest source file | 2,968 lines | 345 lines | Improved and protected by the 345-line blocking threshold; `src/app/services/advisor_brief_service.py` is the evidence-selected largest residual hotspot after extracting performance calculation-evidence completion polling |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 345/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 345-line blocking ceiling.
