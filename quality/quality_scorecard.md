...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current DPM PM operating-quality client-boundary branch keeps the blocking refactor threshold green and ratchets the source-file ceiling to 1,237 script-counted lines. Focused ruff, mypy, upstream-client route tests, DPM client boundary tests, and trial 1,237/1,236-line threshold checks passed. Local `make check` passed with ruff, format check, monetary-float guard, refactor threshold gate, workflow action-runtime gate, mypy over 484 source files, Workbench/OpenAPI contract smoke, and 1,159 unit/contract tests. Local `make ci` passed with 207 integration tests, 1,366 combined coverage tests, 94.29% total coverage, migration contract smoke, and no known vulnerabilities after the governed `PYSEC-2026-161` exception | Keep Docker parity blocking in PR Merge Gate |
...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 1,237-line source-file ceiling; recent slices extracted performance workspace request-context policy, portfolio workspace component parser policy, portfolio readiness response construction, performance evidence artifact handling, portfolio upstream payload handling, removed stale portfolio service wrappers, portfolio source-loading/mapping helpers, and now the DPM PM operating-quality upstream client route family into a dedicated mixin module. `portfolio_service.py` is the largest residual hotspot at 1,237 script-counted lines after reducing `dpm_client.py` from 1,362 to 1,036 physical lines | Continue extraction slices and tighten the source-file ceiling downward |
...
| Tracked `src/app` Python files | 447 | 484 | Added focused portfolio/performance response helpers and workspace/readiness/transaction summary/workflow mapper and contract modules plus risk drawdown, reporting batch, reporting query, risk concentration, risk rolling, risk attribution, performance contribution, performance attribution, performance evidence, portfolio workspace payload, portfolio holdings payload, portfolio catalog payload, position-book mapper, portfolio book response, portfolio workspace source-loading, portfolio readiness/insight source-loading, portfolio book source-loading, advisor-brief source mapper modules, and the DPM PM operating-quality client mixin |
...
| Largest source file | 2,968 lines | 1,237 script-counted lines | Improved and protected by the 1,237-line blocking threshold; `src/app/services/portfolio_service.py` is the largest residual hotspot after extracting the DPM PM operating-quality client route family and reducing `dpm_client.py` below the current ceiling |
