...
| Duplication | 3/5 | The pinned jscpd quality tool now measures 88 production clone findings, 1,767 duplicated lines, and 2.03% duplicated lines, with Python-version-independent AST-scope/canonical-source-side/source-pair/literal-preserving occurrence fingerprints and a no-new-regression ratchet. The protected baseline is generated and enforced on Ubuntu/Node 20; cross-platform candidate-selection reproducibility remains tracked separately. Existing clones remain explicit debt; this slice does not pretend they are cleaned up. | Remove or consolidate reviewed production clones while preserving the ratchet |
