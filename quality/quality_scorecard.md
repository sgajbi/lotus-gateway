...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 329-line source-file ceiling. This slice moves DPM command-center, monitoring, mandate, and portfolio-memory route calls into `src/app/clients/dpm_command_center_client.py` while preserving the public `DpmClient` facade. The evidence-selected largest residual hotspot is now `src/app/clients/advise_proposal_client.py` at 329 lines. | Continue with the next cohesive 329-line hotspot slice |
...
| Largest source file | 2,968 lines | 329 lines | Improved and protected by the 329-line blocking threshold; `src/app/clients/advise_proposal_client.py` is the evidence-selected largest residual hotspot after extracting DPM command-center routes |
