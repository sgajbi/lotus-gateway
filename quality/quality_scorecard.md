...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current portfolio upstream-access extraction branch ratchets the blocking refactor threshold from 1,237 to 1,217 script-counted lines after reducing `portfolio_service.py` below the prior ceiling. Focused ruff, touched-module mypy, portfolio service tests, and the trial refactor threshold gate passed. Local `make check` passed with ruff, format check, monetary-float guard, refactor threshold gate, workflow action-runtime gate, mypy over 487 source files, Workbench/OpenAPI contract smoke, and 1,161 unit/contract tests. Local `make ci` passed with 207 integration tests, 1,368 combined coverage tests, 94.26% total coverage, migration contract smoke, and no known vulnerabilities after the governed `PYSEC-2026-161` exception | Keep Docker parity blocking in PR Merge Gate |
...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 1,217-line source-file ceiling; recent slices extracted performance workspace request-context policy, portfolio workspace component parser policy, portfolio readiness response construction, performance evidence artifact handling, portfolio upstream payload handling, removed stale portfolio service wrappers, portfolio source-loading/mapping helpers, DPM PM operating-quality/construction/proof-pack upstream client route families, and now portfolio cached upstream access into dedicated modules. `dpm_command_center_service.py` is the largest residual hotspot at 1,217 script-counted lines after reducing `portfolio_service.py` from 1,237 to 980 script-counted lines in this slice | Continue extraction slices and tighten the source-file ceiling downward |
...
| Security | 4/5 | Current portfolio upstream-access extraction branch introduces no dependency, authentication, caller-context, monetary-float, or product-error-detail policy changes; local `make ci` found no known vulnerabilities after the governed `PYSEC-2026-161` exception | Triage bandit and sensitive-data handling checks |
...
| Tracked `src/app` Python files | 447 | 487 | Added focused portfolio/performance response helpers and workspace/readiness/transaction summary/workflow mapper and contract modules plus risk drawdown, reporting batch, reporting query, risk concentration, risk rolling, risk attribution, performance contribution, performance attribution, performance evidence, portfolio workspace payload, portfolio holdings payload, portfolio catalog payload, position-book mapper, portfolio book response, portfolio workspace source-loading, portfolio readiness/insight source-loading, portfolio book source-loading, advisor-brief source mapper modules, DPM PM operating-quality/construction/proof-pack client mixins, and the portfolio upstream-access mixin |
...
| Largest source file | 2,968 lines | 1,217 script-counted lines | Improved and protected by the 1,217-line blocking threshold; `src/app/services/dpm_command_center_service.py` is the largest residual hotspot after extracting portfolio cached upstream access and reducing `portfolio_service.py` to 980 script-counted lines |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 1,217-line blocking ceiling.
