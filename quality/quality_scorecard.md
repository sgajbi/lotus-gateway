...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current portfolio holdings-orchestration boundary branch splits portfolio book, liquidity, projected cashflow, allocation, and position-book orchestration into a focused mixin while preserving the public `PortfolioService` method surface. Focused validation passed with 101 portfolio/service-boundary/quality tests. Local `make check` passed with workflow governance, mypy over 567 source files, OpenAPI smoke, and 1,250 unit/contract tests. The quality-baseline workflow now enforces the 575/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Run GitHub gates before merge |
...
| Largest source file | 2,968 lines | 575 script-counted lines | Improved and protected by the 575-line blocking threshold; `src/app/observability/analytics_ui.py` is the single largest residual hotspot after splitting portfolio holdings orchestration |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 575/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 575-line blocking ceiling.
