...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 347-line source-file ceiling. This slice moves DPM command-center, campaign, proof/construction, and wave router aggregation into focused modules while preserving the public `app.router_groups.dpm` exports. The largest residual hotspots are `src/app/contracts/risk_workspace_concentration.py` and `src/app/services/portfolio_holdings_service.py` at 347 lines. | Continue with the next cohesive 347-line hotspot slice |
...
| Largest source file | 2,968 lines | 347 lines | Improved and protected by the 347-line blocking threshold; `src/app/contracts/risk_workspace_concentration.py` is the evidence-selected largest residual hotspot after splitting DPM router aggregation, with `src/app/services/portfolio_holdings_service.py` tied at 347 lines |
...
| Local unit/contract tests | 996 | 1,378 | Added focused proposal lifecycle contract-boundary and lifecycle query service-boundary pins plus focused performance calculation evidence boundary tests, performance workspace context/boundary tests, response-service boundary test, DPM router-group facade and route-family export boundary tests, Workbench rebalance snapshot boundary test, risk drawdown payload-boundary test, performance contribution payload-boundary test, risk workspace response-loading boundary test, and the existing focused contract/service boundary coverage |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 347/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 347-line blocking ceiling.
