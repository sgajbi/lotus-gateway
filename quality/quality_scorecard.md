...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 338-line source-file ceiling. This slice moves risk rolling period-result mapping into `src/app/services/risk_workspace_rolling_periods.py` while preserving rolling response assembly behavior. The largest residual hotspot is now `src/app/observability/analytics_ui.py` at 338 lines. | Continue with the next cohesive 338-line hotspot slice |
...
| Largest source file | 2,968 lines | 338 lines | Improved and protected by the 338-line blocking threshold; `src/app/observability/analytics_ui.py` is the evidence-selected largest residual hotspot after extracting risk rolling period mapping |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 338/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 338-line blocking ceiling.
