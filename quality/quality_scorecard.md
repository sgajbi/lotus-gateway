...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Bank-Buyable service-error config branch keeps the blocking refactor threshold gate green; `make check` passed with ruff, format check, monetary-float guard, the refactor threshold gate, mypy over 472 source files, Workbench/OpenAPI contract smoke, and 1,085 unit/contract tests; `make ci` passed with 207 integration tests, 1,292 coverage tests, 94.11% total coverage, and dependency audit | Keep Docker parity blocking in PR Merge Gate |
...
| Security | 4/5 | Current Bank-Buyable service-error config branch `pip-audit` found no known vulnerabilities after the governed `PYSEC-2026-161` exception; monetary-float guard passed with the existing governed allowlist | Triage bandit and sensitive-data handling checks |
| Documentation | 4/5 | Baseline, scorecard, health report, observability docs, and API-governance docs are refreshed for recent metric-label, reporting error-normalization, shared upstream-error rule, and service-error config hardening | Keep wiki synced and add diagrams over time |
