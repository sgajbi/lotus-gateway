...
| Largest source file | 2,968 lines | 1,804 physical lines | Improved and protected by the 2,000-line blocking threshold; `src/app/services/portfolio_service.py` remains the largest residual hotspot after extracting workspace payload mapping, holdings payload mapping, catalog payload mapping, allocation response mapping, position-book response mapping, transaction-summary context loading, and transaction-ledger assembly; `performance_workspace.py`, `portfolio.py`, `risk_workspace.py`, and `reporting.py` remain smaller residual contract hotspots |
