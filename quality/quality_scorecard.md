...
| --- | ---: | --- | --- |
| Build and test reliability | 4/5 | Current portfolio insights response boundary branch ratchets the blocking refactor threshold from 680 to 667 script-counted lines after moving portfolio insights response assembly into `portfolio_insight_response.py`. Focused validation passed with ruff check, ruff format, mypy over the touched portfolio service/response modules, 43 focused portfolio unit tests, and refactor-threshold trials proving `max_source_file_lines=667` passes while `666` fails on `src/app/services/performance_workspace_horizon.py`. Full local `make check`, `make ci`, live canonical validation, GitHub CI, and post-merge wiki publication are pending for this branch. | Complete full repo-native and live canonical gates before PR merge |
...
| Security | 4/5 | Current portfolio insights response boundary branch introduces no dependency, authentication, caller-context, product-error-detail, monetary-float policy, or upstream error-shape changes; latest full local `make ci` security audit from merged main reported no known vulnerabilities after the governed `PYSEC-2026-161` exception, and this branch's full audit remains pending | Triage bandit and sensitive-data handling checks |
...
| Operations readiness | 3/5 | Existing CI/runbook docs and wiki validation posture now distinguish report-only quality baselines from blocking refactor-threshold and workflow action-runtime gates, including the workflow-level Node 24 JavaScript action runtime opt-in. Latest merged live canonical evidence passed after rebuilding the `lotus-gateway` container from PR #444; this branch still requires a fresh gateway rebuild, canonical Workbench validation, and observability evidence before demo-ready sign-off. | Add incident playbooks and SLO checks |
