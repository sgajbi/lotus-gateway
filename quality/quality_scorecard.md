...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 335-line source-file ceiling. This slice moves the risk attribution payload example into `src/app/contracts/risk_workspace_attribution_examples.py` while preserving the public risk attribution contract surface. The evidence-selected largest residual hotspot is now `src/app/clients/http_resilience.py` at 335 lines, with `risk_workspace_rolling.py` and `performance_workspace_capabilities.py` tied at the same ceiling. | Continue with the next cohesive 335-line hotspot slice |
...
| Largest source file | 2,968 lines | 335 lines | Improved and protected by the 335-line blocking threshold; `src/app/clients/http_resilience.py` is the evidence-selected largest residual hotspot after extracting the risk attribution payload example |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 335/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 335-line blocking ceiling.
