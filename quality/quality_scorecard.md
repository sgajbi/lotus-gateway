...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current proposal service-boundary branch ratchets the blocking refactor threshold from 658 to 646 script-counted lines after moving proposal lifecycle transition orchestration into `proposal_transition_service.py`. Focused validation passed with 42 proposal-service, service-boundary, and threshold tests, and refactor-threshold trials proving `max_source_file_lines=646` passes while `645` fails on `src/app/contracts/advisor_brief.py`. Full local `make check` passed with ruff, format check, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 545 source files, Workbench/OpenAPI contract smoke, and 1,226 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209 integration tests, 1,435 combined coverage tests, 94.32% total coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Run GitHub gates before merge |
| Coverage | 4/5 | 94.32% total coverage across 1,435 combined coverage tests, above the 84% floor in current local `make ci` evidence | Add targeted middleware/security/error tests |
