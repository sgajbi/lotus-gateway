...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current DPM wave campaign-definition contract branch splits campaign-definition request, launch, lifecycle-command, and gateway response contracts into a focused module while preserving the public `app.contracts.dpm_waves` import surface. Focused validation passed with 46 DPM wave contract, service, contract-boundary, and quality-threshold tests. The quality-baseline workflow now enforces the 562/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Local `make check` passed with workflow governance, mypy over 569 source files, OpenAPI smoke, and 1,252 unit/contract tests. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Monitor GitHub feature-lane and quality-baseline gates before merge |
...
| Largest source file | 2,968 lines | 562 script-counted lines | Improved and protected by the 562-line blocking threshold; `src/app/services/workbench_service.py` is the single largest residual hotspot after splitting DPM wave campaign-definition contracts |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 562/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 562-line blocking ceiling.
