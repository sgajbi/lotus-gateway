...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Workbench snapshot-context branch splits Core portfolio/snapshot fan-out, product-safe Core snapshot error mapping, and `WorkbenchSnapshotContext` assembly into a focused module while preserving the public `WorkbenchService` surface. Focused validation passed with 88 Workbench service, snapshot-context, factory, provider, wiki, and quality-threshold tests. The quality-baseline workflow now enforces the 560/49 refactor thresholds as a blocking step while retaining report-only advisory tooling. Local `make check` passed with workflow governance, mypy over 570 source files, OpenAPI smoke, and 1,254 unit/contract tests. Prior branch-local `make ci` passed with migration smoke, 209 integration tests, 1,451 combined coverage tests, 94.30% coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Monitor GitHub feature-lane and quality-baseline gates before merge |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 560/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 560-line blocking ceiling.
