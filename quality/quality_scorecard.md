...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current Bank-Buyable metric-label contract branch keeps the blocking refactor threshold gate green; `make check` passed with ruff, format check, monetary-float guard, the refactor threshold gate, mypy over 472 source files, Workbench/OpenAPI contract smoke, and 1,079 unit/contract tests; `make ci` passed with 207 integration tests, 1,286 coverage tests, 94.07% total coverage, and dependency audit | Keep Docker parity blocking in PR Merge Gate |
...
| Security | 4/5 | Current Bank-Buyable metric-label contract branch `pip-audit` found no known vulnerabilities after the governed `PYSEC-2026-161` exception; monetary-float guard passed with the existing governed allowlist | Triage bandit and sensitive-data handling checks |
