...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 316-line source-file ceiling. This slice moves portfolio workspace section DTOs into `src/app/contracts/portfolio_workspace_sections.py`, keeping profile, reporting, rebalance, and operational-readiness contracts out of the response facade without moving the float-bearing performance summary. The evidence-selected largest residual hotspot is now `src/app/services/foundation_service.py` at 316 lines. | Continue with the next cohesive 316-line hotspot slice |
...
| Largest source file | 2,968 lines | 316 lines | Improved and protected by the 316-line blocking threshold; `src/app/services/foundation_service.py` is the evidence-selected largest residual hotspot after extracting portfolio workspace section contracts |
