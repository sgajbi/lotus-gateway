...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current risk workspace example-boundary branch ratchets the blocking refactor threshold from 709 to 700 script-counted lines after moving response OpenAPI examples into `risk_workspace_examples.py`. Focused validation passed with ruff check, ruff format check, mypy over the touched risk contract modules, 16 focused risk workspace contract/OpenAPI tests, 4 refactor-threshold tests, and refactor-threshold trials proving `max_source_file_lines=700` passes while `699` fails on `src/app/services/dpm_wave_service.py`. Full local `make check` passed with ruff, format check over 739 files, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 524 source files, OpenAPI smoke, and 1,210 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209 integration tests, 1,419 combined coverage tests, 94.29% total coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Keep Docker parity blocking in PR Merge Gate |
...
versus the current risk workspace example-boundary branch after PRs #349 through #438 plus the
current branch.
...
| Largest source file | 2,968 lines | 700 script-counted lines | Improved and protected by the 700-line blocking threshold; `src/app/services/dpm_wave_service.py` is the largest residual hotspot after reducing `src/app/contracts/risk_workspace.py` to 312 script-counted lines |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 700-line blocking ceiling.
