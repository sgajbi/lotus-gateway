...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current portfolio insights response boundary branch ratchets the blocking refactor threshold from 680 to 667 script-counted lines after moving portfolio insights response assembly into `portfolio_insight_response.py`. Focused validation passed with ruff check, ruff format, mypy over the touched portfolio service/response modules, 43 focused portfolio unit tests, and refactor-threshold trials proving `max_source_file_lines=667` passes while `666` fails on `src/app/services/performance_workspace_horizon.py`. Full local `make check` passed with ruff, format check over 750 files, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 531 source files, Workbench contract smoke, and 1,220 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209 integration tests, 1,429 combined coverage tests, 94.31% total coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Keep Docker parity blocking in PR Merge Gate |
| Coverage | 4/5 | 94.31% total coverage across 1,429 combined coverage tests, above the 84% floor in current local `make ci` evidence | Add targeted middleware/security/error tests |
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 667-line source-file ceiling; recent slices extracted Workbench common/overview/sandbox contract families, performance calculation evidence helpers, risk workspace request-context helpers, portfolio workspace contract family, portfolio workflow orchestration, advisor-brief workflow-pack contracts, Advisor Brief client protocols, proposal lifecycle contracts, DPM PM operating-quality contracts, performance workspace context-service policy, portfolio readiness response construction, performance evidence artifact handling, portfolio upstream payload handling, portfolio source-loading/mapping helpers, DPM route families, proposal memo contracts, portfolio liquidity response assembly, Foundation core snapshot parsing, performance horizon-comparison contracts, Advise-owned router groups, advisor-brief AI narrative mapping, risk workspace response OpenAPI examples, analytics workspace-summary payload construction, portfolio catalog response loading, Workbench overview enrichment orchestration, and portfolio insights response assembly into dedicated modules. `src/app/services/portfolio_service.py` is reduced from 680 to 589 script-counted lines, making `src/app/services/performance_workspace_horizon.py` the largest residual hotspot at 667 script-counted lines. | Continue extraction slices and tighten the source-file ceiling downward |
...
| Security | 4/5 | Current portfolio insights response boundary branch introduces no dependency, authentication, caller-context, product-error-detail, monetary-float policy, or upstream error-shape changes; full local `make ci` security audit reported no known vulnerabilities after the governed `PYSEC-2026-161` exception | Triage bandit and sensitive-data handling checks |
...
| Largest source file | 2,968 lines | 667 script-counted lines | Improved and protected by the 667-line blocking threshold; `src/app/services/performance_workspace_horizon.py` is the largest residual hotspot after reducing `src/app/services/portfolio_service.py` to 589 script-counted lines |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 667-line blocking ceiling.
