...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | The proposal decision-vocabulary slice adds a blocking packaged-contract gate to `make lint`, reconciles the current Advise producer artifact in Remote Feature, PR Merge, and Main Releasability, and adds a daily/operator-dispatched drift lane. Pairing drift now reports the changed status/gate and producer blob revision before release. Existing 316/49 refactor, workflow-governance, artifact-integrity, and producer-status controls remain enforced; `scripts/check_agent_quality_evidence.py` continues to guard their durable scorecard and context evidence. | Monitor the protected and scheduled vocabulary gates after push |
