...
| Modularity | 4/5 | Current branch state blocks regressions above the 49-line longest-function baseline and the current 342-line source-file ceiling. This slice moves performance evidence source-supportability parsing into `src/app/services/performance_workspace_evidence_supportability.py` while preserving public evidence facade imports. The largest residual hotspot is now `src/app/services/risk_workspace_rolling.py` at 342 lines. | Continue with the next cohesive 342-line hotspot slice |
...
| Largest source file | 2,968 lines | 342 lines | Improved and protected by the 342-line blocking threshold; `src/app/services/risk_workspace_rolling.py` is the evidence-selected largest residual hotspot after extracting performance evidence supportability parsing |
...
| Local unit/contract tests | 996 | 1,384 | Added focused proposal lifecycle contract-boundary and lifecycle query service-boundary pins plus focused advisor-brief task-flow boundary, advisor-brief review-action boundary, performance calculation evidence boundary/completion tests, performance workspace context/boundary tests, response-service boundary and evidence-supportability boundary tests, portfolio projected-cashflow service-boundary test, risk concentration schema-example boundary test, DPM router-group facade and route-family export boundary tests, Workbench rebalance snapshot boundary test, risk drawdown payload-boundary test, performance contribution payload-boundary test, risk workspace response-loading boundary test, and the existing focused contract/service boundary coverage |
...
3. quality-baseline CI workflow exists and blocks regressions above the refactored 342/49
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 342-line blocking ceiling.
