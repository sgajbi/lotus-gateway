...
| --- | ---: | --- | --- |
| Build and test reliability | 5/5 | Current risk workspace request-context branch ratchets the blocking refactor threshold from 769 to 768 script-counted lines after moving risk request-context dataclasses, as-of date fallback, and context construction beside the risk request builders. Focused validation passed with ruff check, ruff format check, 33 focused risk workspace/request/threshold tests, and the refactor-threshold gate at `max_source_file_lines=768`. Full local `make check` passed with ruff, format check over 729 files, monetary-float guard, refactor-threshold gate, workflow action-runtime gate, mypy over 517 source files, OpenAPI smoke, and 1,191 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209 integration tests, 1,400 combined coverage tests, 94.25% total coverage, and no known vulnerabilities after the governed `PYSEC-2026-161` exception. | Keep Docker parity blocking in PR Merge Gate |
...
| Security | 4/5 | Current risk workspace request-context branch introduces no dependency, authentication, caller-context, or product-error-detail policy changes; monetary-float allowlist remains at 159 governed findings; full local `make ci` security audit reported no known vulnerabilities after the governed `PYSEC-2026-161` exception | Triage bandit and sensitive-data handling checks |
...
| Largest source file | 2,968 lines | 768 script-counted lines | Improved and protected by the 768-line blocking threshold; `src/app/services/portfolio_service.py` is the largest residual hotspot after reducing `src/app/services/risk_workspace_service.py` from 769 to 633 script-counted lines |
...
5. no new function above the current maximum of 49 lines and no Python source file above the
   current 768-line blocking ceiling.
