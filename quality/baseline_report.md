...
guidance synchronized for future agent work.
The current Advisor Brief example-boundary extraction keeps
`scripts/check_agent_quality_evidence.py` aligned with the executable 397/49 refactor ratchet,
current `src/app/services/advisor_brief_service.py` hotspot evidence, and durable
scorecard/context guidance synchronized for future agent work.
The current Advisor Brief response-assembly extraction keeps
`scripts/check_agent_quality_evidence.py` aligned with the executable 397/49 refactor ratchet,
...
scorecard/context guidance synchronized for future agent work.
The current DPM command-center response extraction keeps
...
scorecard/context guidance synchronized for future agent work.
The current DPM PM operating-quality response extraction keeps
...
guidance synchronized for future agent work.
The current upstream error-policy extraction keeps `scripts/check_agent_quality_evidence.py`
...
scorecard/context guidance synchronized for future agent work.
The previous DPM command center core extraction kept
`scripts/check_agent_quality_evidence.py` aligned with the executable 361/49 refactor ratchet,
then-current `src/app/contracts/performance_attribution.py` hotspot evidence, and durable
scorecard/context guidance synchronized for future agent work.
The previous performance attribution trend contract extraction kept
`scripts/check_agent_quality_evidence.py` aligned with the executable 360/49 refactor ratchet,
then-current `src/app/clients/reporting_client.py` hotspot evidence, and durable
scorecard/context guidance synchronized for future agent work.
The previous reporting batch client extraction kept
...
scorecard/context guidance synchronized for future agent work.
The previous DPM exception-summary contract extraction kept
`scripts/check_agent_quality_evidence.py` aligned with the executable 357/49 refactor ratchet,
then-current `src/app/services/foundation_core_snapshot.py` hotspot evidence, and durable
scorecard/context guidance synchronized for future agent work.
The current Foundation core market-value extraction keeps
...
synchronized for future agent work.
The current reporting portfolio contract extraction keeps
...
current `src/app/services/performance_workspace_service.py` hotspot evidence, and durable
scorecard/context guidance synchronized for future agent work.
The current performance workspace response-orchestration extraction keeps
`scripts/check_agent_quality_evidence.py` aligned with the executable 352/49 refactor ratchet,
current `src/app/services/risk_workspace_drawdown.py` hotspot evidence, and durable
scorecard/context guidance synchronized for future agent work.
The current risk drawdown payload-mapping extraction keeps
...
guidance synchronized for future agent work.
The current risk concentration example extraction keeps `scripts/check_agent_quality_evidence.py`
aligned with the executable 347/49 refactor ratchet, current
`src/app/services/portfolio_holdings_service.py` hotspot evidence, and durable scorecard/context
guidance synchronized for future agent work.
The portfolio projected-cashflow extraction kept
...
scorecard/context guidance synchronized for future agent work.
The advisor-brief review-action extraction kept
...
scorecard/context guidance synchronized for future agent work.
The platform capability bootstrap contract extraction keeps
`scripts/check_agent_quality_evidence.py` aligned with the executable 325/49 refactor ratchet,
current evidence-selected `src/app/services/platform_capabilities_service.py` hotspot evidence, and
durable scorecard/context guidance synchronized for future agent work.
The platform capability service cleanup keeps `scripts/check_agent_quality_evidence.py` aligned
with the executable 324/49 refactor ratchet, current evidence-selected
`src/app/services/proposal_service.py` hotspot evidence, and durable scorecard/context guidance
synchronized for future agent work.
The proposal delivery narrative extraction keeps `scripts/check_agent_quality_evidence.py` aligned
with the executable 321/49 refactor ratchet, current evidence-selected
`src/app/services/dpm_client_protocols.py` hotspot evidence, and durable scorecard/context guidance
synchronized for future agent work.
The DPM PM operating-quality protocol extraction keeps `scripts/check_agent_quality_evidence.py`
aligned with the executable 319/49 refactor ratchet, current evidence-selected
`src/app/contracts/portfolio_workspace.py` hotspot evidence, and durable scorecard/context guidance
synchronized for future agent work.
The portfolio workspace section contract extraction keeps
...
Current performance workspace boundary working-tree verification moves detail-view orchestration
into `performance_workspace_detail_views.py`, reducing `performance_workspace_service.py` below the
previous top-file ceiling while preserving the public service surface. Focused validation passed
with 69 targeted performance workspace detail/service-boundary/threshold tests. Full local
`make check` passed with ruff, format check over 769 files, monetary-float guard, refactor threshold
gate, workflow action-runtime baseline, mypy over 548 source files, Workbench/OpenAPI contract smoke,
and 1,231 unit/contract tests. Full local `make ci` passed with migration contract smoke, 209
integration tests, 1,440 combined coverage tests, 94.33% total coverage, and no known vulnerabilities
...
Current risk workspace cache-boundary working-tree verification moves cache-key construction and
replay-time cache-status/correlation stamping into `src/app/services/risk_workspace_cache.py`,
reducing `risk_workspace_service.py` below the source-file ceiling while preserving risk workspace
response behavior. Focused validation passed with ruff check, ruff format check, mypy over the
touched risk workspace service modules, 54 risk workspace cache/service/boundary/threshold tests,
and refactor-threshold trials proving `max_source_file_lines=630` passes while `629` fails only on
`src/app/services/advisory_client_protocols.py`. Full local `make check` passed with ruff, format
check over 772 files, monetary-float guard, refactor threshold gate, workflow action-runtime
...
Current performance contribution payload mapping extraction reduces
`src/app/services/performance_workspace_contribution.py` from 402 to 227 script-counted lines by
moving contribution level, row, position, smoothing-evidence, and source-economics payload mapping
into `src/app/services/performance_workspace_contribution_payloads.py`. The blocking source-file
threshold remains `max_source_file_lines=402` and the executable 402/49 ratchet because
`src/app/services/risk_workspace_service.py` is now the sole 402-line hotspot. Threshold trials
prove 402 passes while 401 fails only on that file. The agent quality evidence gate
`scripts/check_agent_quality_evidence.py` keeps this executable ratchet and hotspot evidence
synchronized with durable scorecard/context guidance.
...
Current risk attribution example extraction moves the static payload example into
`src/app/contracts/risk_workspace_attribution_examples.py` while preserving the public risk
attribution contract import surface. The executable refactor ratchet is now 335/49; the agent
quality evidence gate reports `src/app/clients/http_resilience.py` as the deterministic
evidence-selected largest source file because it is first in sorted scan order among the
335-line tie.

Current HTTP response payload extraction moves response parsing, unsupported-method, and
communication-failure payload shaping into `src/app/clients/http_response_payloads.py`. The
executable refactor ratchet remains 335/49 because `src/app/contracts/risk_workspace_rolling.py`
and `src/app/services/performance_workspace_capabilities.py` remain tied at the ceiling; the agent
quality evidence gate now reports `src/app/contracts/risk_workspace_rolling.py` as the
deterministic evidence-selected largest source file.

Current rolling metric contract extraction moves rolling metric summaries, series points,
series-emission context, and window results into
`src/app/contracts/risk_workspace_rolling_metrics.py` while preserving the public
`risk_workspace_rolling` import surface. The executable refactor ratchet remains 335/49 because
`src/app/services/performance_workspace_capabilities.py` remains at the ceiling and is now the
deterministic evidence-selected largest source file.

Current performance workspace detail capability extraction moves detail capability policy into
`src/app/services/performance_workspace_detail_capabilities.py` and shared module capability
construction into `src/app/services/performance_workspace_module_capability.py` while preserving
the public `performance_workspace_capabilities` import surface. The executable refactor ratchet is
now 334/49, with `src/app/services/foundation_core_snapshot.py` as the deterministic
evidence-selected largest source file.

Current foundation core snapshot section extraction moves payload validation and section parsing
into `src/app/services/foundation_core_snapshot_sections.py` while preserving the public
Foundation mapper behavior. The executable refactor ratchet is now 331/49, with
`src/app/contracts/domain_products.py` as the deterministic evidence-selected largest source file.

Current domain-product graph contract extraction moves graph node, edge, data, and response
contracts into `src/app/contracts/domain_product_graph.py` while preserving the public
`domain_products` import surface. The executable refactor ratchet is now 330/49, with
`src/app/clients/dpm_client.py` as the deterministic evidence-selected largest source file.

Current DPM command-center client extraction moves command-center, monitoring, mandate, and
portfolio-memory route calls into `src/app/clients/dpm_command_center_client.py` while preserving
the public `DpmClient` import surface. The executable refactor ratchet is now 329/49, with
`src/app/clients/advise_proposal_client.py` as the deterministic evidence-selected largest source
file.

Current Advise proposal narrative client extraction moves proposal narrative get, regenerate, and
review route calls into `src/app/clients/advise_proposal_narrative_client.py` while preserving the
public `AdviseClient` import surface. The executable refactor ratchet remains 329/49, with
`src/app/services/workspace_client_protocols.py` as the deterministic evidence-selected largest
source file.

Current proposal delivery narrative extraction moves narrative read/regenerate behavior into
`src/app/services/proposal_delivery_service.py` while preserving the public `ProposalService`
surface. The executable refactor ratchet is now 321/49, with
`src/app/services/dpm_client_protocols.py` as the deterministic evidence-selected largest source
file.

Current DPM PM operating-quality protocol extraction moves the PM quality client protocol family
into `src/app/services/dpm_pm_operating_quality_client_protocols.py` while preserving the public
`DpmCommandCenterClient` protocol surface through focused protocol inheritance. The executable
refactor ratchet is now 319/49, with `src/app/contracts/portfolio_workspace.py` as the
deterministic evidence-selected largest source file.

Current portfolio workspace section contract extraction moves profile, rebalance, reporting, and
operational-readiness DTOs into
`src/app/contracts/portfolio_workspace_sections.py` while preserving the public
`portfolio_workspace` facade and OpenAPI schema names. The executable refactor ratchet is now
316/49, with `src/app/services/foundation_service.py` as the deterministic evidence-selected
largest source file.

Current performance detail-currency correction keeps requested-currency cache isolation while
preventing unsupported independent currency forwarding, reports the effective portfolio base
currency explicitly, and classifies typed currency rejection failures through the shared detail
failure mapper. The executable refactor ratchet remains 316/49, with
`src/app/contracts/performance_workspace_details_contract.py` as the deterministic
evidence-selected largest source file.
