...
1. `src/app/services/foundation_service.py` at 316 lines as the
...
DPM command-center, monitoring, mandate, and portfolio-memory route calls now live in
`src/app/clients/dpm_command_center_client.py`, reducing `src/app/clients/dpm_client.py` below the
current 329-line ceiling while preserving the public `DpmClient` facade.

Advise proposal narrative route calls now live in
...
Platform capability feature, workflow, payload, and module-health pass-through helpers are removed
from `src/app/services/platform_capabilities_service.py`; tests now target the owning
normalization modules directly, reducing the service below the current 324-line ceiling.

Proposal narrative read/regenerate behavior now lives in
...
Portfolio workspace section contracts now live in
`src/app/contracts/portfolio_workspace_sections.py`, reducing
...
`src/app/services/foundation_service.py` hotspot, and durable scorecard/context
