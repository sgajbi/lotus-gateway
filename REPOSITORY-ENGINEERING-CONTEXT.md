...
5. authenticated advisor own-book discovery and bounded own-book value summary are active under
   ...
   interpolations are unresolved, while bare `path`/`url` interpolation and opaque `.format()`
   replacements also fail closed. The seven generic transport exemptions apply only to their
