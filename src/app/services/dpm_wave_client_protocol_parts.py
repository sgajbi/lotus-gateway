"""The lotus-manage wave operations, grouped as the client groups them.

One class described all forty operations and grew past the source-file ratchet
when the tenant was threaded through every campaign call. The split follows the
client's own mixins rather than a line count, so the campaign-workflow contract
lives where the campaign-workflow client does.
"""

from typing import Any, Protocol


class DpmWaveCampaignDefinitionClientProtocol(Protocol):
    """Campaign-definition reads and lifecycle commands."""
...
class DpmWaveCampaignWorkflowClientProtocol(Protocol):
    """Campaign operating-queue, approval, assignment and maker-checker calls."""
...
class DpmWaveCoreClientProtocol(Protocol):
    """The wave aggregate itself: create, read, transition and handoff."""
